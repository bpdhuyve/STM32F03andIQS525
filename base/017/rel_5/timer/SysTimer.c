...
//------------------------------------------------------------------------------------------------//
__irq static void SysTimerIntUpdateIsrTimer1(void);
__irq static void SysTimerIntCompareIsrTimer1(void);
__irq static void SysTimerIntUpdateIsrTimer2(void);
__irq static void SysTimerIntUpdateIsrTimer3(void);
...
U32 SysTimer_Timer_Init(TIMER timer, U32 source_clock_frequency)
{
    ...
    // set prescaler
    timer_reg_ptr->PSC = (tim_clk / source_clock_frequency) - 1;

    // control registers
    timer_reg_ptr->CR1 = 0x0084;
    timer_reg_ptr->CR2 = 0x0000;
    timer_reg_ptr->SMCR = 0x0000;
    timer_reg_ptr->DIER = 0x0000;
    timer_reg_ptr->ARR = 0xFFFF; //default
    timer_reg_ptr->EGR |= 0x0001; //re-init the timer with all configured values

    switch(timer)
    {
    case TIMER_1:
        SysInt_RegisterIsr(TIM1_BRK_UP_TRG_COM_IRQn, (EVENT_CALLBACK)SysTimerIntUpdateIsrTimer1);
        SysInt_RegisterIsr(TIM1_CC_IRQn, (EVENT_CALLBACK)SysTimerIntCompareIsrTimer1);
        SysInt_EnableIsr(TIM1_BRK_UP_TRG_COM_IRQn);
        SysInt_EnableIsr(TIM1_CC_IRQn);
        break;
        
    case TIMER_2:
        SysInt_RegisterIsr(TIM2_IRQn, (EVENT_CALLBACK)SysTimerIntUpdateIsrTimer2);
        SysInt_EnableIsr(TIM2_IRQn);
        break;

    case TIMER_3:
        SysInt_RegisterIsr(TIM3_IRQn, (EVENT_CALLBACK)SysTimerIntUpdateIsrTimer3);
        SysInt_EnableIsr(TIM3_IRQn);
        break;
    }
    
    // return real clock frequency
    return (U32)(tim_clk / (timer_reg_ptr->PSC + 1));
}
//------------------------------------------------------------------------------------------------//
