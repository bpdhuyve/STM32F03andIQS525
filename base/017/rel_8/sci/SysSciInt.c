...
static __irq void SysSciIntUsart3Till6Isr(void);
...
#else
        reg_ptr->CR2 &= ~USART_CR2_LBDIE;
#endif
        reg_ptr->ICR = USART_ICR_LBDCF;
    }
...
BOOL SysSciInt_Channel_Init(SCI_CHANNEL channel)
{
    ...
    if((U8)channel < SYSSCIINT_COUNT)
    {
        ...
        case SCI_CHANNEL_USART3:
            ...
            SysInt_RegisterIsr(USART3_6_IRQn, SysSciIntUsart3Till6Isr);
...
BOOL SysSciInt_Channel_ConfigAsLin(SCI_CHANNEL channel, SCI_CONFIG_STRUCT* config_struct_ptr)
{
    ...
    if(SysSciInt_Channel_Config(channel, config_struct_ptr))
    {
        // ADD LIN SETTINGS
        reg_ptr->CR1 &= ~USART_CR1_UE;  // disable USART;
        reg_ptr->CR2 |= USART_CR2_LBDIE | USART_CR2_LBDL | USART_CR2_LINEN;
        reg_ptr->CR1 |= USART_CR1_UE;   // re-enable USART;
