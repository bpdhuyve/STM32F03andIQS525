...
extern const U32              stack_end_address ;

#if USE_FREERTOS
const U32                       Interrupts[] @ "vector" =
{
    (U32)&stack_end_address,                // The initial stack pointer
...
#else
const U32                       __vector_table[] @ "vector" =
{
    (U32)&stack_end_address,                // The initial stack pointer
    (U32)main,                              // 1 reset handler
    (U32)SysIntHandleNmi,                   // 2 NMI handler
...
{
    // copy essential vectors to SRAM
    MEMSET((VPTR)vector_table, 0, SIZEOF(vector_table));
    MEMCPY((VPTR)vector_table, (VPTR)__vector_table, SIZEOF(__vector_table));
        
    // remap interrupts to SRAM
    // for a STM32F0, one must place the vector table at the beginning of SRAM
    // (which is the physical address of "vector_table") for this to work
    // note that "__vector_table" contains a generic template of the vector table
    // containing the essential vectors only.
    // remappig is needed because:
    // 1. we are not interested in the vectors of the bootloader
    // 2. currently we do not make use of the standard names of IRQ functions for
    //    portability reasons
...
void SysInt_RegisterIsr(ISR_VECTOR vector, ISR_CALLBACK isr_func)
{
    ...
    // Call "SysInt_SetPriority" if executed more than once to set non-zero priority !
}
//------------------------------------------------------------------------------------------------//
