...
static void SysCanIntErrorTask(VPTR unused);
...
static TASK_HNDL                    syscan_error_task_hndl;
...
static void SysCanIntErrorTask(VPTR unused)
{
    // the errors are pushed to the application in low prio to avoid interrupt storm
    SysCanIntErrorIsr();
}
//------------------------------------------------------------------------------------------------//
__irq static void SysCanIntReceiveIsr(void)
...
BOOL SysCanInt_Channel_Init(CAN_CHANNEL channel)
{
    if(channel == CAN_CHANNEL_1) //only one channel
    {
        ...
        //register interrupts
        SysInt_RegisterIsr(CEC_CAN_IRQn, SysCanIntGlobalIsr); //Global ISR
        SysInt_EnableIsr(CEC_CAN_IRQn);

        syscan_error_task_hndl = CoreTask_RegisterTask(1e4, SysCanIntErrorTask, NULL, 255, "SysCanInt Error Task");
        CoreTask_Start(syscan_error_task_hndl);
