...
//------------------------------------------------------------------------------------------------//
#define BUFFER_FORMAT_RGB8_R3G3B2     0
#define BUFFER_FORMAT_RGB16_R5G6B5    1

// other future modes
// - BUFFER_FORMAT_MONOCHROME
// - BUFFER_FORMAT_GRAYSCALE8
// - BUFFER_FORMAT_RGB12_R4G4B4   //hiervan kunnen we nog een 16b aligned variant maken ook indien nodig(+4 ignored bits)
// - BUFFER_FORMAT_RGB24_R8G8B8   //hiervan kunnen we nog een 32b aligned variant maken ook indien nodig(+8 ignored bits)
//================================================================================================//
...
typedef enum
{
    ...
    IMAGE_FORMAT_GRAYSCALE8,      //8bits per pixel
    IMAGE_FORMAT_RGB8_R3G3B2,     //R3G3B2
    IMAGE_FORMAT_RGB16_R5G6B5,    //R5G6B5

    //not suported yet
    //IMAGE_FORMAT_RGB12_R4G4B4
    //IMAGE_FORMAT_RGBA16_R4G4B4A4  //met transparancy
    //IMAGE_FORMAT_RGBA24_R6G6B6A6  //met transparancy
    //IMAGE_FORMAT_RGBA32_R8G8B8A8  //met transparancy
}
IMAGE_FORMAT;
//------------------------------------------------------------------------------------------------//
typedef enum
{
    //todo: dit field wordt eigenlijk niet gebruikt aangezien een font gewoon een refecrence stream is en een image stream, het is de image format die die bepaald hoe een letter getekend wordt, dit veld moet wijzigen naar een soort OPTION FIELD waarbij een eerste optie UTF8 kan zijn
...
FONT;
//------------------------------------------------------------------------------------------------//
/// struct to define a style, you can use this in the code to create a style wich is basicly saying your gonne use that font with those colors
typedef struct
{
    const FONT* font_ptr;
    COLOR front_color;
...
//------------------------------------------------------------------------------------------------//
/// screenbuffer must be inited before you call this function
void StdDraw_Init(U8* screenbuffer_ptr, SIZE screensize, EVENT_CALLBACK flush_callback);

/// calls the flush function, this actually moves all the data to the screen, if this function is never called nothing will be shown on the screen :)
void StdDraw_Flush(void);

/// clears the entire buffer
void StdDraw_Clear(void);

/// clears a region in the buffer
void StdDraw_ClearRegion(REGION region);

/// fills a region in the buffer with a color
void StdDraw_FillRegion(REGION region, COLOR color);

/// calculates the intersection between 2 regions.
/// @param "intersect_result_ptr" pointer to a valid region struct where the intersection info will be placed
...
/// draws an image @ location in the buffer
void StdDraw_DrawImage(POINT location, IMAGE image);

/// draws a specific region of a image at the location, where the location is the origin of the image
void StdDraw_DrawImageCropped(POINT location, IMAGE image, REGION crop_info);

/// draws a text @ location in the screenbuffer
...
/// move a region in the display buffer to a new target location
...
//------------------------------------------------------------------------------------------------//
#define StdDraw_DrawImageRegion(location, image, region)            StdDraw_DrawImageRegion(location, image, region)
//================================================================================================//
