...
static void DrawImage_ReColored_Monochrome(POINT location, IMAGE image, REGION crop_info, COLOR white_replacement_color, COLOR black_replacement_color);
static void DrawImage_ReColored_Grayscale(POINT location, IMAGE image, REGION crop_info, COLOR white_replacement_color, COLOR black_replacement_color);
static void DrawImage_8bColor(POINT location, IMAGE image, REGION crop_info);
static void DrawImage_16bColor(POINT location, IMAGE image, REGION crop_info);
...
static void DrawImage_ReColored_Monochrome(POINT location, IMAGE image, REGION crop_info, COLOR white_replacement_color, COLOR black_replacement_color)
{
    ...
    rows = crop_info.size.height;

    while(rows-- > 0)
    {
        pixels = crop_info.size.width;
...
__ramfunc
#endif
//this function only exist to be called from the DrawText_Styled funtion,
void DrawImage_Recolored(POINT location, IMAGE image, REGION crop_info, COLOR white_replacement_color, COLOR black_replacement_color)
{
    ...
    switch(image.format)
    {
        case IMAGE_FORMAT_MONOCHROME:
            DrawImage_ReColored_Monochrome(location, image, crop_info, white_replacement_color, black_replacement_color);
            break;
        case IMAGE_FORMAT_GRAYSCALE8:
            DrawImage_ReColored_Grayscale(location, image, crop_info, white_replacement_color, black_replacement_color);
            break;
        default:
...
void StdDraw_DrawImageCropped(POINT location, IMAGE image, REGION crop_info)
{
    ...
    {
        case IMAGE_FORMAT_MONOCHROME:
            DrawImage_ReColored_Monochrome(location, image, crop_info, color_White, color_Black);
            break;
        case IMAGE_FORMAT_GRAYSCALE8:
            DrawImage_ReColored_Grayscale(location, image, crop_info, color_White, color_Black);
            break;
        case IMAGE_FORMAT_RGB8_R3G3B2:
            DrawImage_8bColor(location, image, crop_info);
            break;
        case IMAGE_FORMAT_RGB16_R5G6B5:
            DrawImage_16bColor(location, image, crop_info);
            break;
        default:
            LOG_WRN("not supported");
            break;
