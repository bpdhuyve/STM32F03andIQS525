...
//@brief   'data available' bitstatus for data available bitmask in the trace data status/flags byte
#define DATA_AVAILABLE                  0x08

//@brief   'data not available' bitstatus for data available bitmask in the trace data status/flags byte
#define NO_DATA_AVAILABLE               0x00

//@brief   'trace complete' bitstatus for trace status bitmask in the trace data status/flags byte
#define TRACE_COMPLETE                  0x00

//@brief   'waiting for trigger' bitstatus for trace status bitmask in the trace data status/flags byte
#define WAITING_FOR_TRIGGER             0x10

//@brief   'trace busy' bitstatus for trace status bitmask in the trace data status/flags byte
...
//@brief        Function to trigger the tracer externally, with desired nr of samples before and after this trigger.
//@param	pre_samples: the nr of samples to be captured before this trigger event, limited to the max number of
//              samples - 5. This max number of samples is function of the trace buffer size and the word size of the
//              selected trace variables.
//@param	post_samples: the nr of samples to be captured after this trigger event
//              if post_samples = 65535, there is no max pos sample limit: the tracer will continue running until the 
//              trace buffer is full (rate of writing to buffer is higher than reading from buffer)
//@return	TRUE or FALSE if successfull or not
