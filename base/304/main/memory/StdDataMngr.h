...
// @brief   Enumeration with info items that can be retrieved
typedef enum
{
    DATAMNGR_INFO_DATA_ADDRESS          = 0,
    DATAMNGR_INFO_DATA_SIZE             = 1,
    
    // add before + update
    DATAMNGR_INFO_COUNT                 = 2
}
DATAMNGR_INFO;
//================================================================================================//
...
// @brief   Function to be called at the end of the StdDataMngr_LoadDefaults sequence
//          e.g. to allow module defaults to be overwritten by application defaults
...
// @param allow_loaddefaults: allows if the load_defaults_hook can be called for this module on StdDataMngr_LoadDefaults()
// @param module_name       : module name for debugging
...
// @brief   Function to save all data to flash after a given time (DELAY_TIME)
void StdDataMngr_SaveDataDelayed(void);

// @brief   Function to check if data is saved (if last save request is handled completely)
BOOL StdDataMngr_IsDataSaved(void);

// @brief   Function to load all defaults
void StdDataMngr_LoadDefaults(void);

// @brief   Function to (not) allow data saving
void StdDataMngr_AllowSaveData(BOOL allow);

// @brief   Function to get raw data from the data sector
U16 StdDatMngr_GetRawDataPart(U16 offset, U8* buffer_ptr, U16 max_length);

// @brief   Function to set raw data to the data sector
BOOL StdDatMngr_SetRawDataPart(U16 offset, U8* buffer_ptr, U16 buffer_len);

// @brief   Function to get info from StdDataMngr
