    ...
    while(CoreString_GetLength(stringcomposer_item.source_string))
    {
        ...
        else
        {
                    ...
                    (stringcomposer_item.decode_action != PRINT_FIX_CHAR))
            {
                ...
                stringcomposer_item.para_type = (PTYPE)va_arg(argp, U8);
                if(stringcomposer_item.para_type & LOG_BIT_ARRAY)
                {
                    stringcomposer_item.para_len = (U8)va_arg(argp, U8);
                ...
                else
                {
                    ...
                    {
                    case LOG_SIZE_U8:
                        stringcomposer_item.para_data = (U32)va_arg(argp, U8);
                        break;
                    case LOG_SIZE_U16:
                        stringcomposer_item.para_data = (U32)va_arg(argp, U16);
                        break;
                    case LOG_SIZE_U32:
