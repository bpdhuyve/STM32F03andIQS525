       ...
       (DrvMemSysFlash_GetInfo(datamngr_mem_hndl, (U32*)&data_ptr, &memdata_length)))
    {
        ...
        else
        {
            ...
            #if RAM_SIZE_FOR_BACKUP > 0
            {
                ...
                while(memdata_length)
                {
                    copy_len = MIN(memdata_length, RAM_SIZE_FOR_BACKUP);
                    MEMCPY((VPTR)datamngr_backup_ram, data_start_ptr, copy_len);
                    if(DrvMem_WriteData(datamngr_backup_hndl, offset, datamngr_backup_ram, copy_len) == FALSE)
                    {
                        DataMngr_PrintSaveFeedback(FALSE, "make backup");
                        return FALSE;
                    }
                    memdata_length -= copy_len;
                    data_start_ptr += copy_len;
                    offset += copy_len;
                }
                DataMngr_PrintSaveFeedback(TRUE, "make backup");
            }
            #else
            {
...
static BOOL DataMngr_RestoreBackup(void)
{
    ...
    if((datamngr_backup_hndl != NULL) && DrvMemSysFlash_GetInfo(datamngr_backup_hndl, (U32*)&backup_ptr, &memdata_length))
    {
        ...
        #if RAM_SIZE_FOR_BACKUP > 0
        {
            ...
            {
                copy_len = MIN(memdata_length, RAM_SIZE_FOR_BACKUP);
                MEMCPY((VPTR)datamngr_backup_ram, backup_ptr, copy_len);
                if(DrvMem_WriteData(datamngr_mem_hndl, offset, datamngr_backup_ram, copy_len) == FALSE)
                {
                    DataMngr_PrintSaveFeedback(FALSE, "restore backup");
                    return FALSE;
                }
                memdata_length -= copy_len;
                backup_ptr += copy_len;
            ...
            DataMngr_PrintSaveFeedback(TRUE, "restore backup");
        }
        #else
        {
...
void StdDataMngr_SaveData(void)
{
    ...
    for(ram_module_header_ptr = datamngr_module_header; ram_module_header_ptr < &datamngr_module_header[DATAMNGR_MODULE_COUNT]; ram_module_header_ptr++)
    {
        if(DrvMem_WriteData(datamngr_mem_hndl, offset, (U8*)ram_module_header_ptr, SIZE_IN_8BIT(SIZEOF(DATAMNGR_MODULE_HEADER))) == FALSE)
        {
            DataMngr_PrintSaveFeedback(FALSE , "write module headers");
            return;
        }
        offset += SIZE_IN_8BIT(SIZEOF(DATAMNGR_MODULE_HEADER));
    }
    DataMngr_PrintSaveFeedback(TRUE , "write module headers");
