...
static const PIXEL*		biggest_pixel_ptr;
...
static void DrawPixel(PIXEL* pixel_ptr, COLOR color)
{
	...
	if(pixel_ptr >= biggest_pixel_ptr)
	{
		// pixel is out of the reserved memory for the screen so do not set the memeory with this pixel
		return;
	}
...
void StdDraw_Init(U8* screenbuffer, SIZE screensize, EVENT_CALLBACK flush_callback)
{
	...
	biggest_pixel_ptr = &screen_buffer[((screen_region.size.width-1) * (screen_region.size.height-1))];
