...
S32 StdMath_Sine(U32 angle)
{
    ...
    case 0x0000:    // for   0 < x <  90 --> sin(x) = sin(x)
        return (S32)Match_SineTableInterpolation((U16)angle);
    case 0x4000:    // for  90 < x < 180 --> sin(x) = sin(180 - x)
        return (S32)Match_SineTableInterpolation((U16)(0x8000 - angle));
    case 0x8000:    // for 180 < x < 270 --> sin(x) = -sin(x - 180)
        return -(S32)Match_SineTableInterpolation((U16)(angle - 0x8000));
    default:        // for 270 < x < 360 --> sin(x) = -sin(-x)
        return -(S32)Match_SineTableInterpolation((U16)(0 - angle));
    }
}
//------------------------------------------------------------------------------------------------//
...
S32 StdMath_Cosine(U32 angle)
{
    ...
    case 0x0000:    // for   0 < x <  90 --> cos(x) = sin(90 - x)
        return (S32)Match_SineTableInterpolation((U16)(0x4000 - angle));
    case 0x4000:    // for  90 < x < 180 --> cos(x) = -sin(x - 90)
        return -(S32)Match_SineTableInterpolation((U16)(angle - 0x4000));
    case 0x8000:    // for 180 < x < 270 --> cos(x) = -sin(270 - x)
        return -(S32)Match_SineTableInterpolation((U16)(0xC000 - angle));
    default:        // for 270 < x < 360 --> cos(x) = sin(x - 270)
        return (S32)Match_SineTableInterpolation((U16)(angle - 0xC000));
    }
}
//------------------------------------------------------------------------------------------------//
...
//			The excel equivalent formula is: ROUND(SUM(table_ptr)/length;0) and has been tested that way
//------------------------------------------------------------------------------------------------//
S16 StdMath_AvgS16(S16* table_ptr, U16 length)
{
    S32 sum = 0;
    U32 i;
