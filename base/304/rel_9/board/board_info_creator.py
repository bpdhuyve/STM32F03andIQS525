...
def main():
    ...
    if file_name in file_list:
        ...
        address_line += hex(calc_crc(address_line[1:]))[-2:].upper()
        print(address_line)

        #write hardware_number
        hardware_number_line = ":0A" + address[4:] + "00"
        for c in hardware_number:
            hardware_number_line += hex(ord(c))[-2:]
        hardware_number_line += hex(calc_crc(hardware_number_line[1:]))[-2:].upper()
        ...
        #write serial number
        new_address += 0xA
        serial_number_line = ":0C" + str(hex(new_address))[2:].upper() + "00"
        for c in serial_number:
            serial_number_line += hex(ord(c))[-2:]
        serial_number_line += hex(calc_crc(serial_number_line[1:]))[-2:].upper()
        print(serial_number_line)

        #write hardware name
        new_address += 0xC
        hardware_name_line = ":10" + str(hex(new_address))[2:].upper() + "00"
        for c in hardware_name:
            hardware_name_line += hex(ord(c))[-2:]
        for x in range(len(hardware_name), 16):
            hardware_name_line += hex(32)[-2:]
        hardware_name_line += hex(calc_crc(hardware_name_line[1:]))[-2:].upper()
        hardware_name_line = hardware_name_line.upper()
        print(hardware_name_line)

        #write hardware version
        new_address += 0x10
        hardware_version_line = ":0E" + str(hex(new_address))[2:].upper() + "00"
        ...
        hardware_version_line += hex(calc_crc(hardware_version_line[1:]))[-2:].upper()
        ...
        output_file.write(str(hex(int(hardware_function))).zfill(2))
        value = int(hardware_function)
        test = struct.unpack("<I", struct.pack(">I", value))[0]
        output_file.write(str(hex(test)).zfill(2))
