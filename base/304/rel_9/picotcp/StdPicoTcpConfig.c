...
static void PicoTcpConfig_MdnsUpdateHostname(void);
...
extern struct pico_tree         MyRecords;
...
static void PicoTcpConfig_LoadStoreDefaults(void)
{
    picotcp_store.ip_address[0] = 192;
    picotcp_store.ip_address[1] = 168;
    picotcp_store.ip_address[2] = 100;
    picotcp_store.ip_address[3] = 150;
    
    picotcp_store.ip_mask[0]    = 255;
    picotcp_store.ip_mask[1]    = 255;
    picotcp_store.ip_mask[2]    = 255;
    picotcp_store.ip_mask[3]    = 0;
    
    picotcp_store.ip_gateway[0] = 192;
    picotcp_store.ip_gateway[1] = 168;
    picotcp_store.ip_gateway[2] = 100;
    picotcp_store.ip_gateway[3] = 1;
    
    picotcp_store.dhcp_active   = FALSE;
...
static void PicoTcpConfig_MdnsCheckReinit(void)
{
    if(mdns_reinit_needed)
    {
        ...
        // delete all records from MyRecords
        PICO_MDNS_RTREE_DESTROY(&MyRecords);
        SetClaimIdCount(0);
        
        if(pico_mdns_reinit(picotcp_store.hostname, pico_ipv4_link_by_dev(emac)->address, &PicoTcpConfig_MdnsInitCallback, NULL) != 0)
        {
            LOG_WRN("[mDNS] initialisation failed");
        }
    }
...
static void PicoTcpConfig_MdnsRegisterService(void)
{
    ...
    // service name
    MEMSET((VPTR)temp_buffer, 0, SIZEOF(temp_buffer));
    CoreString_CopyString("PSI_",temp_buffer);
    for(i=3; i<6; i++)
    {
        CoreConvert_U8ToHexString(picotcp_data.mac_address[i], &temp_buffer[2*(i-3)+4], FALSE, TRUE);
    }
...
static void PicoTcpConfig_MdnsUpdateHostname(void)
{
    // delete previous hostname from MyRecords  
    PICO_MDNS_RTREE_DESTROY(&MyRecords);
    SetClaimIdCount(0);
    
    // set new hostname
    if(pico_mdns_set_hostname(picotcp_store.hostname, NULL) != 0)
    {
        LOG_WRN("[mDNS] set hostname failed");
        return;
    }

    LOG_DBG("[mDNS] set hostname to: %s", PCSTR(picotcp_store.hostname));
}
//================================================================================================//
...
#ifdef MEM_MEASURE
    LOG_TRM(" PICO mem          act %d - hwm %d - max %d", PU32(cur_mem), PU32(max_mem), PU32((U32)&__ICFEDIT_size_heap__));
#endif    
    CoreTerm_PrintAcknowledge();
}
//------------------------------------------------------------------------------------------------//
...
BOOL StdPicoTcpConfig_SetParameterAsString(PICOTCPCONFIG_PARA parameter, STRING string_value)
{
    ...
    case PICOTCPCONFIG_PARA_HOSTNAME:
        ...
        PicoTcpConfig_MdnsUpdateHostname();
        break;
...
{
    struct mem_chunk_stats*     stats = (struct mem_chunk_stats *)((U8*)x - sizeof(struct mem_chunk_stats));
    U32                         size_with_stats = stats->size;
    
    if(x == NULL)
    {
        LOG_WRN("PICOTCP free NULL pointer");
        picotcp_data.pico_app_error = MEM_ERROR_FREE_NULL;
        return;
    }
    
    if(stats->signature != 0xdeadbeef)
    {
        LOG_WRN("PICOTCP free signature error");
        picotcp_data.pico_app_error = MEM_ERROR_FREE_NULL;
        return;
    }
    ...
    if(x != stats->mem)
    {
        LOG_WRN("PICOTCP free pointer error");
        picotcp_data.pico_app_error = MEM_ERROR_FREE_NULL;
        return;
    }
    
    memset(stats, 0, size_with_stats);
    cur_mem -= size_with_stats;
...
{
    struct mem_chunk_stats*     stats = NULL;
    U32                         size_with_stats = x + sizeof(struct mem_chunk_stats);

    /* check for -1 malloc */
    if (x == 0xffffffff)
    {
        LOG_WRN("PICOTCP alloc size -1");
        picotcp_data.pico_app_error = MEM_ERROR_ALLOC_FAILED;
        return NULL;
    }
    
    /* allocated size + stats */
    stats = (struct mem_chunk_stats*)malloc(size_with_stats);
    if(stats == NULL)
    {
        LOG_WRN("PICOTCP alloc failed");
        picotcp_data.pico_app_error = MEM_ERROR_ALLOC_FAILED;
        return NULL;
    }
    ...
    stats->mem = ((U8*)stats) + sizeof(struct mem_chunk_stats);
    stats->size = size_with_stats;
