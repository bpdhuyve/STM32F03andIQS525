...
static S16                      msec_trace_buffer[STDGENTRACER_NB_OF_BUFFERS][STDGENTRACER_MAX_TRACE_CHANNELS];
...
static TRACE_SPEC               trace_specs[STDGENTRACER_MAX_TRACE_CHANNELS];
static TRIGGER_SPEC             trigger_specs[STDGENTRACER_MAX_TRIGGER_CHANNELS];
...
static void StdGenTracer_StartTrace(void)
{
   ...
   if(length != 0)
   {
       ...
       //function div_s requires numerator < denumerator
       //so we shift the denumerator till it is greater than the numerator
       //div_s performs numerator / denumerator * 32768
       //so after div_s we have to divide the result by (15 - nr_of_shifts)
      /* while(length < STDGENTRACER_BUFFER_LENGTH)
       {
           length = length << 1;
           nr_of_shifts++;
       }
       maximum_number_of_records = div_s(STDGENTRACER_BUFFER_LENGTH, length);
       length >>= nr_of_shifts;
       nr_of_shifts = 15 - nr_of_shifts;
       maximum_number_of_records = maximum_number_of_records >> nr_of_shifts;*/
...
static void StdGenTracer_CheckTrigger(void)
{
    ...
    if(trigger_status == TRIGGER_NOT_FOUND)
    {
        /*
           ...
           ------------------------------------------------------------------------------
        */
        if(((trigger_specs[0].status & USED_UNUSED_MASK) != UNUSED) && (trigger_mode != NO_TRIGGER))
        {
            i = 0;
        ...
        {
            /*
            if no trigger is specified then start tracing until buffer is full.
            The trigger-status is set to TRIGGER_FOUND and the trigger-mode
            is set to NO_TRIGGER.
            -------------------------------------------------------------------
            */
