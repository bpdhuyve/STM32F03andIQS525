...
{
    LOG_DEV("DrvCanChannel_NotifyTxMessageReady");
...
void DrvCanChannel_FillExtDataFrameMssgStruct(CAN_MSSG_STRUCT* mssg_ptr, U32 identifier, U8 dlc, U8* data_ptr)
{
    ...
    if(dlc > 8) dlc = 8;
    mssg_ptr->dlc = dlc;
    if(dlc > 0) MEMCPY((VPTR)&mssg_ptr->data[0], (VPTR)data_ptr, dlc);
}
//------------------------------------------------------------------------------------------------//
void DrvCanChannel_FillExtDataFrameMuxMssgStruct(CAN_MSSG_STRUCT* mssg_ptr, U32 identifier, U8 mux, U8 data_length, U8* data_ptr)
{
    ...
    mssg_ptr->identifier = identifier;
    mssg_ptr->data[0] = mux;
    if(data_length > 7) data_length = 7;
    mssg_ptr->dlc = data_length + 1;
    if(data_length > 0) MEMCPY((VPTR)&mssg_ptr->data[1], (VPTR)data_ptr, data_length);
}
//================================================================================================//
