...
typedef void (*ANGLE_ON_INDEX_PULSE)(U32 angle_at_index_pulse);

typedef void (*ANGLE_SET_ABSOLUTE_POSITION_AT_INDEXPULS)(ANGLE_ID angle_id, U32 angle);
...
typedef struct
{
    ...
    ANGLE_SET_ABSOLUTE_POSITION_AT_INDEXPULS    set_absolute_position_at_indexpuls;   
}
ANGLE_HOOK_LIST;
...
//          will give you an callback every time the angle is reached
BOOL DrvAngle_RegisterHookOnRelativePosition(ANGLE_HNDL hndl, U16 angle, EVENT_VPTR_CALLBACK hook,  VPTR data_ptr);

// @remark  If the angle is in the past, it is possible, but not sure, the hook will called from this
//          registration context.  This behaviour is SysLib dependant.
//          will give you an callback once when the position is reached      
...
// @remark  If the index puls happens the absolute position will be changed to positon you give
void DrvAngle_ChangeAbsolutePostionOnIndexPuls(ANGLE_HNDL hndl, U32 angle);

// @remark  will delete all registerd OnAbosutePosition and OnRelativePosition that will occur in the future
