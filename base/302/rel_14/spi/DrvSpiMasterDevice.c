...
BOOL DrvSpiMasterDevice_Select(SPI_DEVICE_ID device_id)
{
    ...
    if(device_id < spi_device_count)
    {
        ...
        if(DrvSpiMasterChannel_Config(spi_dev_hndl->spi_channel_hndl, &(spi_dev_hndl->config_struct)))
        {
            ...
            LOG_DBG("B CRH %08h", PU32(((GPIO_TypeDef*)GPIOB_BASE)->CRH));
            LOG_DBG("B CRL %08h", PU32(((GPIO_TypeDef*)GPIOB_BASE)->CRL));
            LOG_DBG("B IDR %08h", PU32(((GPIO_TypeDef*)GPIOB_BASE)->IDR));
            LOG_DBG("AFIO %08h", PU32(((AFIO_TypeDef*)AFIO_BASE)->MAPR));
