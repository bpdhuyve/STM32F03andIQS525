...
//------------------------------------------------------------------------------------------------//
static SENSOR_HOOK_LIST				sensor_se95_hook_list;
...
static BOOL DrvSensorNxpSe95_ReadSensor(SENSOR_ID sensor_id, SENSOR_TYPE sensor_type, BOOL wait_to_complete)
{
	...
	if((sensor_type == SENSOR_TYPE_TEMPERATURE) && (se95_ctrl_ptr->i2c_is_present) && (sensor_se95_active == INVALID_SENSOR_ID))
	{
...
{
	sensor_se95_hook_list.is_present_hook = DrvSensorNxpSe95_IsPresent;
	sensor_se95_hook_list.read_sensor_hook = DrvSensorNxpSe95_ReadSensor;
	sensor_se95_hook_list.get_value_hook = DrvSensorNxpSe95_GetValue;
...
SENSOR_HNDL DrvSensorNxpSe95_Register(I2C_CHANNEL_HNDL i2c_channel, U8 address)
{
	...
	if(sensor_se95_count < SE95_COUNT)
	{
		...
		sensor_struct_ptr->hook_list_ptr = &sensor_se95_hook_list;
