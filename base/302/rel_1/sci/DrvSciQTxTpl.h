...
static BOOL                             Q_PREFIX(_tx_active) = FALSE;
//================================================================================================//
...
static U8 Q_PREFIX(SciQTx_GetNextByteHook)(U8* byte_ptr, U8 length)
{
    ...
    Q_PREFIX(_tx_active) = (BOOL)(count > 0);
    if(Q_PREFIX(_tx_active))
    {
        if(count > length)
        {
            count = length;
...
static void Q_PREFIX(SciQTx_NotifyTxDataReady)(void)
{
    if((Q_PREFIX(_tx_channel_hndl) != NULL) && (Q_PREFIX(_tx_active) == FALSE));
    {
