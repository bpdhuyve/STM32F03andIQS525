...
//================================================================================================//
#define MEM_DRVMEMMX25L12835FM2I_C
//================================================================================================//
...
#ifndef MEM_DRVMEMMX25L12835FM2I_C_LOG_LEVEL
	#define CORELOG_LEVEL               LOG_LEVEL_DEFAULT
#else
	#define CORELOG_LEVEL               MEM_DRVMEMMX25L12835FM2I_C_LOG_LEVEL
#endif
//------------------------------------------------------------------------------------------------//
// @brief  Defines the maximum number of blocking SPI Master channels
#ifndef DRVMEMMX25L12835FM2I_COUNT
	#define DRVMEMMX25L12835FM2I_COUNT		1
#endif
//------------------------------------------------------------------------------------------------//
#ifndef DRVMEMMX25L12835FM2I_IGNORE_FLASH_CLEAR_FAIL
	#define DRVMEMMX25L12835FM2I_IGNORE_FLASH_CLEAR_FAIL		0
#endif
//------------------------------------------------------------------------------------------------//
#ifndef DRVMEMMX25L12835FM2I_NUMBER_OF_SECTORS
    #define DRVMEMMX25L12835FM2I_NUMBER_OF_SECTORS               4096 //32000
#endif
//------------------------------------------------------------------------------------------------//
...
#endif
//------------------------------------------------------------------------------------------------//
#ifndef DRVMEMMX25L12835FM2I_SPI_CLOCK_SPEED
    #define DRVMEMMX25L12835FM2I_SPI_CLOCK_SPEED             20000000
#endif
//================================================================================================//
...
#include "mem\DrvMemM25L12835FM2I.h"
...
typedef enum //TODO: Check commands
{                                   // Description                          address bytes    - dummy bytes   - data bytes
    ...
    COMMAND_SE              = 0x20, // Sector Erase                             3                   0               0
    COMMAND_CE              = 0x60, // Chip Erase                               0                   0               0
    ...
    COMMAND_RDP             = 0xAB, // Release from deep power down             0                   0               0
}
COMMAND;
//================================================================================================//
...
static void Command_MX25L128ReadId(void);
static void Command_MX25L128ReadStatus(void);
static void Command_MX25L128Clear(void);
static void Command_MX25L128Read(void);
static void Command_MX25L128Write(void);
#endif

static BOOL DrvMemMX25L128_WaitUntilReady(MEM_ID mem_id);
static BOOL DrvMemMX25L128_IsReady(MEM_ID mem_id);
static U8 DrvMemMX25L128_ReadStatus(MEM_ID mem_id);

static BOOL DrvMemMX25L128_Clear(MEM_ID mem_id);
static BOOL DrvMemMX25L128_Read(MEM_ID mem_id, U32 address_offset, U8* data_ptr, U16 length);
static BOOL DrvMemMX25L128_Write(MEM_ID mem_id, U32 address_offset, U8* data_ptr, U16 length);
static BOOL DrvMemMX25L128_Verify(MEM_ID mem_id, U32 address_offset, U8* data_ptr, U16 length);
static BOOL DrvMemMX25L128_Flush(MEM_ID mem_id);
//================================================================================================//
...
//------------------------------------------------------------------------------------------------//
static const MEM_HOOK_LIST                  mem_hook_list = {DrvMemMX25L128_Clear, DrvMemMX25L128_Read, DrvMemMX25L128_Write, DrvMemMX25L128_Verify, DrvMemMX25L128_Flush};
static MEM_STRUCT                           mem_struct[DRVMEMMX25L12835FM2I_COUNT];
static MEM_CTRL_STRUCT                      mem_ctrl_struct[DRVMEMMX25L12835FM2I_COUNT];
static U8                                   mem_count;
...
static void Command_MX25L128ReadStatus(void)
{
    ...
    LOG_TRM("%02x", PU8(DrvMemMX25L128_ReadStatus(mem_id)));
    CoreTerm_PrintAcknowledge();
}
//------------------------------------------------------------------------------------------------//
static void Command_MX25L128Clear(void)
{
    ...
    CoreTerm_PrintFeedback(DrvMemMX25L128_Clear(mem_id));
}
//------------------------------------------------------------------------------------------------//
static void Command_MX25L128Read(void)
{
    ...
    if(DrvMemMX25L128_Read(mem_id, CoreTerm_GetArgumentAsU32(1), test_buffer, length))
    {
...
static void Command_MX25L128Write(void)
{
    ...
    CoreTerm_PrintFeedback(DrvMemMX25L128_Write(mem_id, CoreTerm_GetArgumentAsU32(1), test_buffer, length));
}
#endif
//------------------------------------------------------------------------------------------------//
static BOOL DrvMemMX25L128_WaitUntilReady(MEM_ID mem_id)
{
    ...
    while(DrvMemMX25L128_IsReady(mem_id) == FALSE)
    {
        System_KickDog();
...
static BOOL DrvMemMX25L128_IsReady(MEM_ID mem_id)
{
    return (BOOL)((DrvMemMX25L128_ReadStatus(mem_id) & 0x01) == 0x00);
}
//------------------------------------------------------------------------------------------------//
static U8 DrvMemMX25L128_ReadStatus(MEM_ID mem_id)
{
...
static BOOL DrvMemMX25L128_Clear(MEM_ID mem_id)
{
    ...
    for(offset = 0; offset < mem_ctrl_struct_ptr->length; offset += DRVMEMMX25L12835FM2I_SECTOR_SIZE)
    {
        System_KickDog();

        // check if ready
        if(DrvMemMX25L128_WaitUntilReady(mem_id) == FALSE)
        {
            return FALSE;
        }
...
static BOOL DrvMemMX25L128_Write(MEM_ID mem_id, U32 address_offset, U8* data_ptr, U16 length)
{
    ...
    while(length > 0)
    {
        ...
        // determine (remaining)
        page_length = DRVMEMMX25L12835FM2I_PAGE_SIZE - (page_address & 0x000000FF);
...
void DrvMemMX25L128_Init(void)
{
    ...
    CoreTerm_RegisterCommand("MX25L128ReadId", "MX25L128 Read ID (a: mem_id)", 1, Command_MX25L128ReadId, TRUE);
    CoreTerm_RegisterCommand("MX25L128ReadStatus", "MX25L128 Read Status (a: mem_id)", 1, Command_MX25L128ReadStatus, TRUE);
    CoreTerm_RegisterCommand("MX25L128Clear", "MX25L128 Clear (a: mem_id)", 1, Command_MX25L128Clear, TRUE);
    CoreTerm_RegisterCommand("MX25L128Read", "MX25L128 Read (a: mem_id, b:addr_offset, c:length)", 3, Command_MX25L128Read, TRUE);
    CoreTerm_RegisterCommand("MX25L128Write", "MX25L128 Write (a: mem_id, b:addr_offset, c:length, d:data_byte)", 4, Command_MX25L128Write, TRUE);
}
//------------------------------------------------------------------------------------------------//
MEM_HNDL DrvMemMX25L128_Register(SPI_DEVICE_ID spi_device_id, U32 address, U16 sector_count)
{
    ...
    U32                 length = sector_count * DRVMEMMX25L12835FM2I_SECTOR_SIZE;
    ...
    if(((address >> 16) + sector_count) > DRVMEMMX25L12835FM2I_NUMBER_OF_SECTORS)
    {
    ...
    if(mem_count < DRVMEMMX25L12835FM2I_COUNT)
    {
        config_struct.speed     = DRVMEMMX25L12835FM2I_SPI_CLOCK_SPEED;
...
{
    SPI_DEVICE_ID spi_device_id = mem_ctrl_struct[mem_hndl->mem_id].spi_device_id;
    
    if(mem_hndl->mem_id >= mem_count)
    {
        LOG_WRN("Illegal MEM_HNDL");
        return FALSE;
    }
    
    spi_data[0] = (U8)COMMAND_DP;
