...
BOOL DrvAngle_RegisterHookOnAngle(ANGLE_HNDL hndl, U32 angle, EVENT_VPTR_CALLBACK hook,  VPTR data_ptr)
{
    ...
    for(U8 i =0; i < ANGLE__DRV_COMPARE_BUFFER; i++)
    {
         if(angle_ctrl_pntr->angles_buffer[i].state == NOT_ACTIVE)
         {
             ...
             angle_ctrl_pntr->angles_buffer[i].angle = angle;  //starts on the absolute position angle
             angle_ctrl_pntr->angles_buffer[i].void_pointer = data_ptr;
             DrvAngle_SetNextCompareAngleIfNeeded(hndl, angle, &angle_ctrl_pntr->angles_buffer[i]);
             return TRUE;
         }
    }
...
BOOL DrvAngle_RegisterHookOnAbsolutePosition(ANGLE_HNDL hndl, U32 angle, EVENT_VPTR_CALLBACK hook, VPTR data_ptr)
{
    ...
    for(U8 i =0; i < ANGLE__DRV_COMPARE_BUFFER; i++)
    {
         if(angle_ctrl_pntr->angles_buffer[i].state == NOT_ACTIVE)
         {
             ...
             angle_ctrl_pntr->angles_buffer[i].angle_reached_hook = hook;
             angle_ctrl_pntr->angles_buffer[i].angle = angle;
             angle_ctrl_pntr->angles_buffer[i].void_pointer = data_ptr;
             DrvAngle_SetNextCompareAngleIfNeeded(hndl, angle, &angle_ctrl_pntr->angles_buffer[i]);
             return TRUE;
         }
...
void DrvAngle_ClearAllRegisterOnAbsolutePosition(ANGLE_HNDL hndl)
{
    ...
    if(angle_ctrl_pntr->current_angle_pntr->state != ON_ANGLE)
    {
        angle_ctrl_pntr->current_angle_pntr = NULL;
    }
}
//================================================================================================//
