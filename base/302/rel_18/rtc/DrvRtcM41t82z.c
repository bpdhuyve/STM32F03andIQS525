...
//------------------------------------------------------------------------------------------------//
BOOL DrvRtcM41t82z_ClearHaltBit(I2C_DEVICE_ID i2c_device_id)
{
    U8  rtc_i2c_buffer[2];    
    U8* address_buf_ptr = &rtc_i2c_buffer[0];
    U8* registers_buf_ptr = &rtc_i2c_buffer[1];
    ...
    *address_buf_ptr = 0x0C;
    ...
    // read register
    if(DrvI2cMasterDevice_ReadData(rtc_i2c_hndl[i2c_device_id], registers_buf_ptr, CLOCK_REGISTER_COUNT, TRUE))
    {
