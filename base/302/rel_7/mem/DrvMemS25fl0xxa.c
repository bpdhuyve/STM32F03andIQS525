...
static BOOL DrvMemS25fl0xxa_Clear(MEM_ID mem_id)
{
    ...
    for(offset = 0; offset < mem_ctrl_struct_ptr->length; offset += S25FL0XXA_SECTOR_SIZE)
    {
        System_KickDog();
...
static BOOL DrvMemS25fl0xxa_Write(MEM_ID mem_id, U32 address_offset, U8* data_ptr, U16 length)
{
    ...
    while(length > 0)
    {
        ...
        // determine (remaining)
        page_length = S25FL0XXA_PAGE_SIZE - (page_address & 0x000000FF);
...
MEM_HNDL DrvMemS25fl0xxa_Register(SPI_DEVICE_ID spi_device_id, U32 address, U8 sector_count)
{
    ...
    U32                 length = sector_count * S25FL0XXA_SECTOR_SIZE;
