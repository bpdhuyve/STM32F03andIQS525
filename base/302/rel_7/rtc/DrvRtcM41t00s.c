...
static BOOL DrvRtcM41t00s_GetTime(RTC_ID rtc_id, RTC_TIME* rtc_time_ptr)
{
    ...
    // set register address 0
    rtc_i2c_buffer[0] = 0;
    if(DrvI2cMasterDevice_WriteData(i2c_hndl, rtc_i2c_buffer, 1, TRUE) == FALSE)
    {
        return FALSE;
    }
    
    // read time
    if(DrvI2cMasterDevice_ReadData(i2c_hndl, rtc_i2c_buffer, 8, TRUE))
    {
        rtc_time_ptr->second        = Rtc_ConvertHexToDec(rtc_i2c_buffer[0] & 0x7F);
        rtc_time_ptr->minute        = Rtc_ConvertHexToDec(rtc_i2c_buffer[1] & 0x7F);
        rtc_time_ptr->hour          = Rtc_ConvertHexToDec(rtc_i2c_buffer[2] & 0x3F);
        rtc_time_ptr->day_of_week   = (DAY_OF_WEEK)((rtc_i2c_buffer[3] & 0x07) - 1);
        rtc_time_ptr->day_of_month  = Rtc_ConvertHexToDec(rtc_i2c_buffer[4] & 0x3F);
        rtc_time_ptr->month         = (MONTH)Rtc_ConvertHexToDec(rtc_i2c_buffer[5] & 0x1F);
        rtc_time_ptr->year          = 2000 + 100*((rtc_i2c_buffer[2] >> 6) & 0x01) + Rtc_ConvertHexToDec(rtc_i2c_buffer[6] & 0xFF);
        
        // check for oscillator failure
        if(rtc_i2c_buffer[1] & 0x80)
        {
            // kick oscillator
            rtc_i2c_buffer[0] = 0;
            rtc_i2c_buffer[1] = 0x80 + Rtc_ConvertDecToHex(rtc_time_ptr->second);
            ...
            return DrvI2cMasterDevice_WriteData(i2c_hndl, rtc_i2c_buffer, 2, TRUE);
        }
        return TRUE;
    }
    return FALSE;
