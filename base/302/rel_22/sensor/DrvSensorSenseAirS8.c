...
#ifndef SENSEAIRS8_COUNT
	#define SENSEAIRS8_COUNT            1
#endif
//================================================================================================//
...
//------------------------------------------------------------------------------------------------//
static const SENSOR_HOOK_LIST	    sensor_senseairs8_hook_list = {DrvSensorSenseAirS8_IsPresent, DrvSensorSenseAirS8_ReadSensor, DrvSensorSenseAirS8_GetValue};
static U8							sensor_senseairs8_count;
static SENSEAIRS8_CTRL				sensor_senseairs8_ctrl[SENSEAIRS8_COUNT];
static SENSOR_STRUCT				sensor_senseairs8_struct[SENSEAIRS8_COUNT];
...
static BOOL DrvSensorSenseAirS8_IsPresent(SENSOR_ID sensor_id)
{
    ...
    if(sensor_id >= sensor_senseairs8_count)
    {
        return FALSE;
    }
...
static BOOL DrvSensorSenseAirS8_ReadSensor(SENSOR_ID sensor_id, SENSOR_TYPE sensor_type, BOOL wait_to_complete)
{
    ...
    if(sensor_type == SENSOR_TYPE_CO2)
    {
        ...
        senseairs8_ctrl_ptr->successfull_reading = FALSE;
    }
    else
    {
        return FALSE;
    ...
    if(sensor_senseairs8_active == INVALID_SENSOR_ID)
    {
        ...
        if(wait_to_complete)
        {
            ...
            senseairs8_ctrl_ptr->successfull_reading = TRUE;
        }
        else
        {
...
static BOOL DrvSensorSenseAirS8_GetValue(SENSOR_ID sensor_id, SENSOR_TYPE sensor_type, U16* data_ptr)
{
    ...
    if((sensor_type == SENSOR_TYPE_CO2) && (senseairs8_ctrl_ptr->is_present))
    {
        *data_ptr = senseairs8_ctrl_ptr->co2_level;
        return senseairs8_ctrl_ptr->successfull_reading;
    }
	return FALSE;
}
//------------------------------------------------------------------------------------------------//
...
SENSOR_HNDL DrvSensorSenseAirS8_Register(SCI_CHANNEL_HNDL sci_channel)
{
	...
	if(sensor_senseairs8_count < SENSEAIRS8_COUNT)
	{
		...
		sensor_struct_ptr->hook_list_ptr = (SENSOR_HOOK_LIST*)&sensor_senseairs8_hook_list;
