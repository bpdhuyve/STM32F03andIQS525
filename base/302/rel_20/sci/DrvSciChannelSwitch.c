...
static BOOL InCircularRange(U16 index, U16 min, U16 max);
//================================================================================================//
        ...
        if (registered_channels_data[channel_id].read_index == write_index)
        {
            ...
            do
            {
                ...
                if (write_index >= DRVSCICHANNELSWITCH_BUFFER_LENGTH)
                    write_index -= DRVSCICHANNELSWITCH_BUFFER_LENGTH;  
                
                total_num_bytes_fetched += num_bytes_fetched;   
                
            } while (total_num_bytes_fetched < length && num_bytes_fetched > 0);
            
            
            // update read indexes of other channels when they're overrun by the write index
            for (j = 0; j < num_registered_channels; j++)
            {
                if (j != channel_id)
                {
                    if (InCircularRange(registered_channels_data[j].read_index, write_index - total_num_bytes_fetched, write_index))
                    {
                        // the write index overran the read index of this channel
                        // move the read index of this channel just past the write index
                        registered_channels_data[j].read_index = write_index +1;
                        if (registered_channels_data[j].read_index >= DRVSCICHANNELSWITCH_BUFFER_LENGTH)
                            registered_channels_data[j].read_index -= DRVSCICHANNELSWITCH_BUFFER_LENGTH;
                    }
                }
            }
...
//------------------------------------------------------------------------------------------------//
BOOL DrvSciChannelSwitch_AddChannel(SCI_CHANNEL_HNDL sci_channel_hndl)
{
    U8 i;
    ...
    {
        if (registered_channels_data[i].registered_drvscichannelswitch_channel_hndl == sci_channel_hndl)
        {
           // already registered
          return TRUE;
        }
        ...
        {
            registered_channels_data[i].registered_drvscichannelswitch_channel_hndl = sci_channel_hndl;
            registered_channels_data[i].read_index = 0;
            
            DrvSciChannel_RegisterTxHook(sci_channel_hndl, list_of_drvsciswitch_get_next_byte_hooks[i]);
            DrvSciChannel_RegisterRxHook(sci_channel_hndl, DrvSciChannelSwitch_OnNewByteFromSci);
