...
static void WriteConfigurationBank()
{
    ...
    for(i = 0; i < NR_OF_USED_STAGES; i++ )
    {
        ...
        WriteRegister((current_start_address+4), stage_offset_registers[i]);//offset low
        ...
        WriteRegister((current_start_address+7), (U16)(stage_offset_registers[i]*1.25));//offset low clamp     datasheet zegt Set STAGEx_OFFSET_LOW register to 80% of the STAGEx_OFFSET_LOW_CLAMP value. vandaar *1.25
...
void DrvSliderAd7147_PrintDebugData(void)
{
    ...
    //log
    LOG_TRM("|=====================================================|");
    LOG_TRM("|Pad Nr  |Ambient |measured|relvalue|relthres|touched |");
    LOG_TRM("|--------|--------|--------|--------|--------|--------|");

    for ( i = 0; i < NR_OF_USED_STAGES; i++ )
    {
        LOG_TRM("|%8d|%8d|%8d|%8d|%8d|%8d|",PU16(i),PU16(ambient_values[i]),PU16(measured_values[i]),PU16(pad_values[i]),PU16(threshhold_values[i]),PU8((touch_mask>>i)&0x01));
    }
    LOG_TRM("|=====================================================|");


//    //print out also counter to see if the calibration is being forced
//    StdLogIPrint("force calib cnt : ");
//    StdLogIPrintNumber(force_recalibration_counter);
//    StdLogIPrintLn("");
}
//================================================================================================//
