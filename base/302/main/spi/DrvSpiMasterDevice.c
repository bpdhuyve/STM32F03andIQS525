...
typedef struct
{
    ...
    SPI_CONFIG_STRUCT   config_struct; // speed, clk_mode, bitcount, lsb_first(BOOL)
...
//local array of active   SPI_device(s)_struct(s):
...
void DrvSpiMasterDevice_Init(void)
{
    ...
    spi_device_count = 0;
    //reserve memory for the local array of spi_device_struct(s)
    MEMSET((VPTR)spi_device_struct, 0, SIZEOF(spi_device_struct));

    //registration (@DrvSpiMasterChannel) of my callback hook
...
*@note	
*@param	spi_channel_hndl
*       pin_cs: handle to chip select pin
*       config_struct_ptr: optional (NULL = no ptr)
*                           @arg struct determines: speed, clk_mode, bitcount, lsb_first(BOOL)
*       msg_complete: optional (NULL = no ptr)
*@retval  SPI_DEVICE_ID is a U8 (range 0 to SPI_DEVICE_COUNT) 
**/      	
                                          ...
                                          EVENT_CALLBACK msg_complete)
{
    ...
    //handle to the next free array element in: spi_device_struct[SPI_DEVICE_COUNT]
    ...
    {   
        ...
        spi_device_count++;
        // make sure the new SPI device is not selected (SPI_nSS = HIGH)
...
//------------------------------------------------------------------------------------------------//
/**
*@brief    general SPI write and/or read function of an un-selected spi_device
*/
