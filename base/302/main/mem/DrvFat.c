...
U32 get_fattime (void)
{
    ...
    if(rtc_time_hndl == NULL)
    {
        date_time  = ((year - 1980) & 0x7F) << 25;
        date_time +=  (month        & 0x0F) << 21;
        date_time +=  (day          & 0x1F) << 16;
        date_time +=  (hour         & 0x1F) << 11;
        date_time +=  (min          & 0x3F) << 05;
        date_time +=  (sec          & 0x1F) << 00;
    }
    else if(DrvRtc_GetLocalTime(rtc_time_hndl, &rtc_time))
    {
        date_time  = ((rtc_time.year - 1980) & 0x7F) << 25;
        date_time +=  (rtc_time.month        & 0x0F) << 21;
        date_time +=  (rtc_time.day_of_month & 0x1F) << 16;
        date_time +=  (rtc_time.hour         & 0x1F) << 11;
        date_time +=  (rtc_time.minute       & 0x3F) << 05;
        date_time +=  (rtc_time.second/2     & 0x1F) << 00;        
    }
