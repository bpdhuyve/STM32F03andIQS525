...
typedef BOOL (*SCI_NOTITY)(SCI_CHANNEL_ID channel_id);
...
// LIN
typedef BOOL (*SCI_REGISTER_LIN_BREAK_DETECT_HOOK)(SCI_CHANNEL_ID channel_id, EVENT_CALLBACK lin_break_detected_hook);
...
typedef struct
{
    ...
    SCI_NOTITY                      notify_tx_data_ready;
    // MPCM
    SCI_CONFIG_AS_MPCM              config_as_mpcm_hook;
    SCI_SET_MPCM_FILTER_HOOK        set_mpcm_filter_hook;
    // LIN
    SCI_CONFIG                      config_as_lin_hook;
    SCI_REGISTER_LIN_BREAK_DETECT_HOOK  lin_break_detected_hook;
    SCI_NOTITY                      send_lin_break_hook;
}
