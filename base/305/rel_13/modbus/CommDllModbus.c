...
typedef struct
{
    ...
    U8                                  commdllmodbus_frame_bytes[STDDLL_FRAME_LENGTH];
    ...
    TASK_HNDL                           commdllmodbus_1_5_task;
    ...
    if(!modbus_channel_ptr->commdllmodbus_sci_tx_pipe_active)
    {
        while(length)
        {
            ...
            case DLL_MODBUS_DEC_STATE_RECEIVE_SLAVE_ADDR:
                modbus_channel_ptr->commdllmodbus_frame_nok = 0;
                ...
                *input_data_ptr = *byte_ptr;
                input_data_ptr++;
                CoreTask_Start(modbus_channel_ptr->commdllmodbus_1_5_task);
            ...
            case DLL_MODBUS_DEC_STATE_RECEIVE_DATA: 
                ...
                {
                    CoreTask_Stop(modbus_channel_ptr->commdllmodbus_1_5_task);
                    CoreTask_Stop(modbus_channel_ptr->commdllmodbus_3_5_task);
                    if(modbus_channel_ptr->commdllmodbus_frame_nok == 0)
                    {
                        input_length++;
                        *input_data_ptr = *byte_ptr;
                        input_data_ptr++;
...
static void CommDllModbus_1_5Task(VPTR data_ptr)
{
    ...
    CoreTask_Stop(modbus_channel_ptr->commdllmodbus_1_5_task);
    modbus_channel_ptr->commdllmodbus_frame_nok++;
}
//------------------------------------------------------------------------------------------------//
static void CommDllModbus_3_5Task(VPTR data_ptr)
{
    ...
    CoreTask_Stop(modbus_channel_ptr->commdllmodbus_3_5_task);
    if(modbus_channel_ptr->commdllmodbus_frame_nok == 1)
    {
        modbus_channel_ptr->commdllmodbus_frame_nok--;
    ...
    else
    {
        ...
        modbus_channel_ptr->commdllmodbus_error = DLLMODBUS_ERR_INFRAME_SILENT_INTERVALS;
    }
}
//------------------------------------------------------------------------------------------------//
static U16 CRCGeneration(U8* value_ptr, U8 length)
...
static void CommDllModbus_Handler_For_Channel(MODBUS_CHANNEL modbus_channel)
{
    ...
    case DLL_MODBUS_STATE_RX:
        if(modbus_channel_ptr->commdllmodbus_new_data_ready)
        {
            ...
            LOG_DBG("PHY to DLL: %02h", PU8A(modbus_channel_ptr->commdllmodbus_frame_bytes, modbus_channel_ptr->commdllmodbus_length));
    ...
    case DLL_MODBUS_STATE_WAIT_BEFORE_TX:
            ...
            LOG_DBG("DLL to PHY: %02h", PU8A(modbus_channel_ptr->commdllmodbus_frame_bytes, modbus_channel_ptr->commdllmodbus_length));
...
{    
    ...
    time_1_5 = (U32)((interchar_delay * 15) /10);
    time_1_5 = (time_1_5 - (time_1_5 % 1000)) + 1001;
    time_3_5 = (U32)((interchar_delay * 35) /10);
    time_3_5 = (time_3_5 - (time_3_5 % 1000)) + 1001;
    
    LOG_DEV("interchar delay (1.5) = %d", PU32(time_1_5));
    LOG_DEV("interframe delay (3.5) = %d", PU32(time_3_5));
        
    if ((modbus_channel_ptr->commdllmodbus_1_5_task != NULL) &&
        (modbus_channel_ptr->commdllmodbus_3_5_task != NULL))
    {
        CoreTask_SetPeriod(modbus_channel_ptr->commdllmodbus_1_5_task, time_1_5);
        CoreTask_SetPeriod(modbus_channel_ptr->commdllmodbus_3_5_task, time_3_5);
    }
    else
    {
        return FALSE;
...
MODBUS_CHANNEL CommDllModbus_RegisterChannel(SCI_CHANNEL_HNDL channel_hndl, SCI_CONFIG_STRUCT* sci_config_ptr)
{
    ...
    if (dllmodbus_channel_count < MODBUS_CHANNEL_COUNT) 
    {
        ...
        {         
            ...
            modbus_channel_ptr->commdllmodbus_1_5_task = CoreTask_RegisterTask(0, CommDllModbus_1_5Task, modbus_channel_ptr, 255, "Modbus_1_5");
            modbus_channel_ptr->commdllmodbus_3_5_task = CoreTask_RegisterTask(0, CommDllModbus_3_5Task, modbus_channel_ptr, 255, "Modbus_3_5");
