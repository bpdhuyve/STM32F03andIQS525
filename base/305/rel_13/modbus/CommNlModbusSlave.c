...
typedef struct
{
    ...
    U8                          commnlmodbus_reply_data[STDNL_FRAME_LENGTH];
...
static void CommNlModbusSlave_Timeout(VPTR data_ptr);
//================================================================================================//
...
static void CommNlModbusSlave_HandleMsg(MODBUS_CHANNEL modbus_channel, U8* data_ptr, U8 length)
{
    ...
    else
    {
        if((modbus_nl_channel_ptr->commnlmodbus_request_hook != NULL) && (data_ptr[0] != 0x00))
        {
            ...
            if (!modbus_nl_channel_ptr->commnlmodbus_timedout)
            {
            ...
            {
                LOG_WRN("[MODBUS NL SLAVE] Time out");
                modbus_nl_channel_ptr->commnlmodbus_timedout = FALSE;
            }
        }
...
static void CommNlModbusSlave_Timeout(VPTR data_ptr)
{
    MODBUS_NL_SLAVE_STRUCT* modbus_nl_channel_ptr = (MODBUS_NL_SLAVE_STRUCT*)data_ptr;
    
    CoreTask_Stop(modbus_nl_channel_ptr->commnlmodbus_timeout_task);
    
    modbus_nl_channel_ptr->commnlmodbus_timedout = TRUE;
}
//================================================================================================//
...
void CommNlModbusSlave_RegisterModbusChannel(MODBUS_CHANNEL modbus_channel, U8 address)
{
    ...
    modbus_nl_channel_ptr->commnlmodbus_timeout_task = CoreTask_RegisterTask(T_TIMEOUT_SLAVE, CommNlModbusSlave_Timeout, modbus_nl_channel_ptr, 127, "ModbusTimeOut");
    modbus_nl_channel_ptr->commnlmodbus_timedout = FALSE;
