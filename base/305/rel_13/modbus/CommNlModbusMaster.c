...
#include "modbus\CommNlModbusMaster.h"
...
static void CommNlModbusMaster_HandleMsg(MODBUS_CHANNEL modbus_channel, U8* data_ptr, U8 length)
{
    ...
    LOG_DBG("[MODBUS NL MASTER] Frame received: %02h", PU8A(data_ptr, length));
...
//------------------------------------------------------------------------------------------------//
void CommNlModbusMaster_RegisterResponseHook(MODBUS_CHANNEL modbus_channel, EVENT_VPTR_CALLBACK response_hook)
{
    VALIDATE_AND_GET_MODBUS_NL_CHANNEL_PTR(modbus_channel, TRUE);
    
    modbus_nl_channel_ptr->commnlmodbus_response_hook = response_hook;
