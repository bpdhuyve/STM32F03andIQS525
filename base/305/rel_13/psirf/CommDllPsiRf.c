...
#endif
static void CommDllPsiRf_Handler(void);
//================================================================================================//
...
static DLLPSIRFHOOK_FRAME_RECEIVED      dllpsirf_log_hook;
...
static void DllPsiRfHandleRxMessage(DLL_PSIRF_MSG* inbox_msg_ptr)
{
    if(dllpsirf_log_hook != NULL)
    {
        dllpsirf_log_hook(&inbox_msg_ptr->buffer[0], inbox_msg_ptr->rssi);
    }
...
static void DllPsiRfLoadTxMessage(void)
{
    ...
    if(CoreQ_Read(dllpsirf_outbox_q_hndl, (VPTR)&dllpsirf_outbox_msg, 1))
    {
        if(dllpsirf_log_hook != NULL)
        {
            dllpsirf_log_hook((U8*)&dllpsirf_outbox_msg, 0xFF);
        }
                       ...
                       DRVGPIO_PIN_HNDL gdo2_pin_hndl)
{
    ...
    Core_RegisterModuleHandler(CommDllPsiRf_Handler);
