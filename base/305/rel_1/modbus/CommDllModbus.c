...
#endif
//------------------------------------------------------------------------------------------------//
// @brief  Defines the time in µs, before this time there must be an answer to a request ohterwhise you get timeout
#ifndef T_TIMEOUT
...
static void CommDllModbus_RxNewByte(U8* byte_ptr, U8 length)
{
    ...
    {
        LOG_WRN("RX char while not in RX mode: 0x02h", PU8(byte));
...
void CommDllModbus_Init(SCI_CHANNEL_HNDL channel_hndl, SCI_SPEED sci_speed)
{
    ...
    case SCI_SPEED_19200_BPS:
        time_1_5 = 859;             //11-bit (= 1 start + 8 data + even parity + 1 stop) * (1/sci speed) * 1,5
        time_3_5 = 2005;            //11-bit (= 1 start + 8 data + even parity + 1 stop) * (1/sci speed) * 3,5
        break;
    default:						//vanaf 38400bps
        time_1_5 = 730; //430
        time_3_5 = 1750; //1003
        break;
    }
