...
static void NlPsiRfPrintMsgStruct(STRING prestring, NL_PSIRF_MSG* msg_ptr);
...
static void NlPsiRfOnTimeOut(VPTR data_ptr)
{
    ...
    case NL_PSIRF_WAIT_FOR_INCLUSION:
        ...
        NlPsiRfOnEntry(NL_PSIRF_NO_ADDRESS, 0);
        //report failure
        NlPsiRfReportMsgSuccess(FALSE);     // inclusion failed
        break;
    case NL_PSIRF_WAIT_FOR_EXCLUSION:
        //exit current state
        NlPsiRfOnExit();
        ...
        //report failure
        NlPsiRfReportMsgSuccess(FALSE);     // exclusion failed
        break;
    case NL_PSIRF_WAIT_FOR_BROADCAST_FORWARD_TIMEOUT:
        //exit current state
        NlPsiRfOnExit();
    ...
    case NL_PSIRF_WAIT_FOR_ACK_SET_NID:
        ...
        NlPsiRfOnExit();
        //side effect & next state
        if((nlpsirf_outgoing_msg.length > STDNLPSIRF_HEADER_LENGTH_BASE) && (nlpsirf_outgoing_msg.data[STDNLPSIRF_HEADER_EXTRA_INCL] > 0))
        {
...
static void NlPsiRfSendMsg(NL_PSIRF_MSG* msg_ptr)
{
    ...
    NlPsiRfPrintMsgStruct("[SND MSG]", msg_ptr);
...
static void NlPsiRfPrintMsgStruct(STRING prestring, NL_PSIRF_MSG* msg_ptr)
{
    ...
    if(msg_ptr->version == MSG_VERSION_V0)
    {
        LOG_DBG("%s %s - %08x : %02d->%02d (%02d->%02d) - %2d 0x%02x - (%d) %02x",
                PCSTR(prestring),
...
static U8 NlPsiRfDetermineRouteTo(U8 rtnid)
{
    ...
    {
        if(nlpsirf_data_ptr->routing_table[rtnid][0] > 0)
        {
...
static NL_PSIRF_MSG_CATEGORY NlPsiRfCategorizeMsg(NL_PSIRF_MSG* msg_ptr)
{
    if(msg_ptr->msg_type.v0 == MSG_TYPE_V0_EXPLORE)
    {
        if((nlpsirf_data_ptr->network_hid > 0) &&
           ((nlpsirf_in_linking == TRUE) || (nlpsirf_data_ptr->network_hid == msg_ptr->hid)))
        {
    ...
    else if(msg_ptr->hid == nlpsirf_data_ptr->network_hid)
    {
        ...
        else if((msg_ptr->tnid == 0) && (msg_ptr->rtnid == 0))
        {
            ...
            else if((msg_ptr->msg_type.v0 == MSG_TYPE_V0_SET_NID) && (msg_ptr->length == STDNLPSIRF_HEADER_LENGTH_BASE))
            {
...
static BOOL NlPsiRfHandleReceivedMsg(U8* frame_ptr, U8 rssi)
{
    ...
    {
        NlPsiRfPrintMsgStruct("[RCV MSG]", &incoming_msg);
        ...
        case MSG_CATEGORY_BC_NEW:
            ...
            case NL_PSIRF_WAIT_FOR_EXCLUSION:
                //report failure
                NlPsiRfReportMsgSuccess(FALSE);     // exclusion failed
                //check if failure report changed categorisation
                if(NlPsiRfCategorizeMsg(&incoming_msg) != MSG_CATEGORY_BC_NEW)
                {
                    return FALSE;
                }
                break;
        ...
        case MSG_CATEGORY_P2P_SELF:
            ...
            case MSG_TYPE_V0_SET_NID:
                ...
                if(nlpsirf_data_ptr->network_address != STDBLPSIRF_NETWORK_MASTER_ADDR)
                {
                    if((incoming_msg.length <= STDNLPSIRF_HEADER_LENGTH_INCL) || (incoming_msg.data[STDNLPSIRF_HEADER_EXTRA_INCL] == 0))
                    {
                        ...
                        NlPsiRfOnExit();
                        //side effects
                        NlPsiRfComposeAck(&incoming_msg);			//send acknowledge before you clear network id en network address
                    ...
                    else if((incoming_msg.data[STDNLPSIRF_HEADER_EXTRA_INCL] > STDBLPSIRF_NETWORK_MASTER_ADDR) && (nlpsirf_state == NL_PSIRF_WAIT_FOR_INCLUSION))
                    {
                        //exit current state
                        NlPsiRfOnExit();
                        //side effects
						...
						NlPsiRfComposeAck(&incoming_msg);	//send acknowledge after you fill in network id en network address
                        if(nlpsirf_set_nid_hook != NULL)
                        {
            ...
            case MSG_TYPE_V0_LINK:
                ...
                else if((nlpsirf_data_ptr->network_address == STDBLPSIRF_NETWORK_MASTER_ADDR) && (incoming_msg.data[0] <= 1))
                {
        ...
        {
            NlPsiRfPrintMsgStruct("[RPL MSG]", &incoming_msg);
    ...
    if((nlpsirf_state == NL_PSIRF_NO_ADDRESS) || (nlpsirf_state == NL_PSIRF_IDLE))
    {
        ...
        else if(nlpsirf_data_ptr->network_address == STDBLPSIRF_NETWORK_MASTER_ADDR)
        {
            ...
            else
            {
                for(i=STDBLPSIRF_NETWORK_MASTER_ADDR+1; i<STDNLPSIRF_MAX_NODES; i++)
                {
                    if(nlpsirf_data_ptr->routing_table[i][0] == STDBLPSIRF_NETWORK_MASTER_ADDR)      // unknown route to node
                    {
                        LOG_DBG("NL - Search Node %d", PU8(i));
                        outbox_msg.msg_id = NlPsiRfGetNextMsgToSendId();
                        outbox_msg.msg_type.v0 = MSG_TYPE_V0_LINK;
                        outbox_msg.rtnid = 0;
                        outbox_msg.data[0] = i;
                        outbox_msg.data_length = 1;
                        msg_to_handle = TRUE;
                        nlpsirf_data_ptr->routing_table[i][0] = 0;
                        break;
                    }
                }
            }
...
void CommNlPsiRf_PrintStatus(void)
{
    ...
    LOG_TRM("NL State       : %s", PCSTR(nlpsirf_states[nlpsirf_state]));
