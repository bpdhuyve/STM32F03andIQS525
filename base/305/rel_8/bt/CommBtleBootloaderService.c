...
//------------------------------------------------------------------------------------------------//
#define UPLOAD_DATA_Q_SIZE              300
#define UPLOAD_DATA_BUFFER_SIZE         30
...
typedef struct
{
    ...
    PRODUCT_VERSION             active_software;
    PRODUCT_VERSION             backup_software;
    NEW_SOFTWARE_STRUCT         new_software;
    MEM_HNDL                    mem_hndl;
    U32                         max_image_length;
...
static const U8                 comm_ctrl_data_invalid = INVALID_RESPONSE;
...
//------------------------------------------------------------------------------------------------//
static PRODUCT_VERSION GetBackupSoftwareVersion(void)
{
    ...
    DrvMem_ReadData(comm_upload_ctrl.mem_hndl, 0, (U8*)&flash_header, SIZEOF(SWHEADER));
    DrvMem_ReadData(comm_upload_ctrl.mem_hndl, BTLE_BOOTLOADER_PSILDR_OFFSET, (U8*)&psildr_header, PSILDR_HEADER_SIZE);
    
    if((flash_header.version.product_number == comm_upload_ctrl.active_software.product_number) &&
       (flash_header.crc == StdCrcGenerateCrc16IBM((U8*)&flash_header, SIZEOF(SWHEADER) - 2)) &&
       (psildr_header.start_of_header == 0x48) &&
       (psildr_header.header_crc == StdCrcGenerateCrc16IBM((U8*)&psildr_header, PSILDR_HEADER_SIZE - 2)) &&
       (psildr_header.header_version == 1))
    {
        // Headers are valid, check on data in psildr file
        crc_encrypted = 0xFFFF;
        crc_decrypted = 0xFFFF;
        for(offset = 0; offset < psildr_header.data_length; offset += length)
        {
            length = MIN(PSILDR_BUFFER_SIZE, (psildr_header.data_length - offset));
            
            DrvMem_ReadData(comm_upload_ctrl.mem_hndl, PSILDR_HEADER_SIZE + BTLE_BOOTLOADER_PSILDR_OFFSET + offset, comm_psildr_buffer, length);
            
            crc_encrypted = StdCrcGenerateCrc16IBMPart(comm_psildr_buffer, (U32)length, crc_encrypted);
            ...
            crc_decrypted = StdCrcGenerateCrc16IBMPart(comm_psildr_buffer, (U32)length, crc_decrypted);
        }
        if((crc_encrypted == psildr_header.data_crc) &&
           (crc_decrypted == psildr_header.reserved_bytes))
        {
            return flash_header.version;
        }
    }
    
    flash_header.version.major_revision = 0xFFFF;
    flash_header.version.minor_revision = 0xFFFF;
    flash_header.version.product_number = 0xFFFF;
    flash_header.version.test_revision  = 0xFFFF;
    return flash_header.version;
}
//------------------------------------------------------------------------------------------------//
...
//------------------------------------------------------------------------------------------------//
static void ValidateNewImage(void)
{
    BOOTLDR_APP_RAW_HEADER psildr_header;
    SWHEADER flash_header;
    U32 offset;
       ...
       (crc_decrypted == psildr_header.reserved_bytes))
    {
        ...
        if(DrvMem_WriteData(comm_upload_ctrl.mem_hndl, 0, (U8*)&flash_header, SIZEOF(SWHEADER)))
        {
            ...
            comm_upload_ctrl.backup_software  = comm_upload_ctrl.new_software.version;
...
static void HandleStartDfuMessage(U8* data_ptr, U8 length)
{
    ...
    if((new_software.product_number != comm_upload_ctrl.active_software.product_number) ||
       (new_software_length > comm_upload_ctrl.max_image_length))
    {
        RespondNotOk(OPCODE_START_DFU);
        return;
    }
    
    // Is software already located on the target?
    if(AreSoftwaresEqual(new_software, comm_upload_ctrl.active_software) ||
       AreSoftwaresEqual(new_software, comm_upload_ctrl.backup_software))
    {
        LOG_TRM("%s%s software already on target", PCSTR(module_name), PCSTR(GetProcedureString(OPCODE_START_DFU)));
        RespondNotOk(OPCODE_START_DFU);
        return;
    }
    ...
    MEMSET(&comm_upload_ctrl.backup_software, 0xFF, SIZEOF(comm_upload_ctrl.backup_software));
...
//------------------------------------------------------------------------------------------------//
static void HandleValidateDataMessage(U8* data_ptr, U8 length)
{
    LOG_DBG("%sHandleValidateDataMessage - %02h", PCSTR(module_name), PU8A(data_ptr, length));
...
static void HandleUploadDataMessage(U8* data_ptr, U8 length)
{
    ...
    {
        // TODO: ermee stoppen
        CommBtleBootloaderService_RegisterCtrlPointData((U8*)&comm_ctrl_data_invalid, 1);
...
static void Command_BtleUploadInfo(void)
{
    ...
    LOG_TRM("Active software:       %d.%d.%d.%d", PU16(comm_upload_ctrl.active_software.product_number),
                                                  PU16(comm_upload_ctrl.active_software.major_revision),
                                                  PU16(comm_upload_ctrl.active_software.minor_revision),
                                                  PU16(comm_upload_ctrl.active_software.test_revision));
    LOG_TRM("Backup software:       %d.%d.%d.%d", PU16(comm_upload_ctrl.backup_software.product_number),
                                                  PU16(comm_upload_ctrl.backup_software.major_revision),
                                                  PU16(comm_upload_ctrl.backup_software.minor_revision),
                                                  PU16(comm_upload_ctrl.backup_software.test_revision));
    ...
    LOG_TRM("Max image length:      %d", PU32(comm_upload_ctrl.max_image_length));
...
//------------------------------------------------------------------------------------------------//
void CommBtleBootloaderService_Init(MEM_HNDL mem_hndl, PRODUCT_VERSION product_software, U32 max_image_length, const U16* encryption_table_ptr)
{
    StdEncryptInitDecoder((U16*)encryption_table_ptr);
    
    comm_upload_data_q_hndl           = CoreQ_Register(UPLOAD_DATA_Q_SIZE, 1, "Upload data q");
    ...
    comm_upload_ctrl.last_procedure   = OPCODE_INVALID;
    comm_upload_ctrl.active_software  = product_software;
    comm_upload_ctrl.max_image_length = max_image_length;
    comm_upload_ctrl.validation_state = VALIDATION_NOT_DONE;
    comm_upload_ctrl.backup_software  = GetBackupSoftwareVersion();
    
    comm_is_tx_free_hook              = NULL;
    comm_notify_ctrl_hook             = NULL;
    comm_activate_image_hook          = NULL;
    
    if((comm_upload_ctrl.active_software.product_number == comm_upload_ctrl.backup_software.product_number) &&
       ((comm_upload_ctrl.active_software.major_revision != comm_upload_ctrl.backup_software.major_revision) ||
        (comm_upload_ctrl.active_software.minor_revision != comm_upload_ctrl.backup_software.minor_revision) ||
        (comm_upload_ctrl.active_software.test_revision  != comm_upload_ctrl.backup_software.test_revision)))
    {
        EnterState(UPLOAD_DONE);
    }
    else
...
void CommBtleBootloaderService_Handler(void)
{
    ...
    // Check if current upload needs to be pauzed/canceled
...
//------------------------------------------------------------------------------------------------//
BOOL CommBtleBootloaderService_IsBackupImageValid(void)
{
    PRODUCT_VERSION software;
    
    software = GetBackupSoftwareVersion();
    return (BOOL)(software.product_number == comm_upload_ctrl.active_software.product_number);
}
//------------------------------------------------------------------------------------------------//
void CommBtleBootloaderService_RegisterCtrlPointData(U8* data_ptr, U16 length)
{
    ...
    case CTRL_DATA_PENDING:
        LOG_WRN("Previous CMD still pending!");
...
void CommBtleBootloaderService_RegisterUploadData(U8* data_ptr, U16 length)
{
    ...
    {
        LOG_DBG("%sReceiving upload data in state %s", PCSTR(module_name), PCSTR(state_names[comm_upload_ctrl.state]));
        return;
    }
