...
//------------------------------------------------------------------------------------------------//
#define DEVICE_RX_STRING_LENGTH     50
//================================================================================================//
...
//------------------------------------------------------------------------------------------------//
void Task_BT_Send(VPTR ptr);
BOOL SendCommand(STRING command);
BOOL SendCommand_U8Argument(STRING command, U8 arg1);
BOOL SendCommand_StringArgument(STRING command, STRING arg1);
void TermCommand_BtDeleteBondInfo(void);
...
//------------------------------------------------------------------------------------------------//
MODULE_DECLARE();
STRING                              device_tx_string;
STRING                              convert_string;
STRING                              device_rx_string;
STRING                              device_rx_char;
STRING                              bt_rx_string;

SCI_CHANNEL_HNDL                    uart_hndl;
SCI_CONFIG_STRUCT                   uart_config;

EVENT_PASSKEY_RECEIVED              event_passkey_received = NULL;
EVENT_CALLBACK                      event_device_state_changed = NULL;
EVENT_CALLBACK                      event_bt_data_received = NULL;
EVENT_SWVERSION_RECEIVED            event_swversion_received = NULL;
EVENT_CHIPID_RECEIVED               event_chipid_received = NULL;

DEVICE_STATE                        bt_device_state = STATE_UNKOWN;
//================================================================================================//
...
BOOL SendCommand(STRING command)
{
    ...
    BT_TX_Q_SciQTx_Write((U8*)device_tx_string,CoreString_GetLength(device_tx_string));
    return TRUE;
}
//------------------------------------------------------------------------------------------------//
BOOL SendCommand_U8Argument(STRING command, U8 arg1)
...
void CommBtUart_Init(SCI_CHANNEL_HNDL uart_hndl)
{
    ...
    device_rx_string = CoreBuffer_NewString(50,"CommBtUart device_rx_string"); //bt rx string 1 packet can max be 20 bytes + de bt commands, het passkey cmd is langer
