...
static DLLPSIRFHOOK_FRAME_RECEIVED      dllpsirf_frame_hook[2];
static U8                               dllpsirf_frame_hook_count;
...
static void DllPsiRfHandleRxMessage(DLL_PSIRF_MSG* inbox_msg_ptr)
{
    ...
    for(i=0; i< dllpsirf_frame_hook_count; i++)
    {
        if(dllpsirf_frame_hook[i] != NULL)
        {
            if(dllpsirf_frame_hook[i](&inbox_msg_ptr->buffer[0], inbox_msg_ptr->rssi))
            {
                       ...
                       DRVGPIO_PIN_HNDL gdo2_pin_hndl)
{
    ...
    dllpsirf_frame_hook[0] = NULL;
    dllpsirf_frame_hook[1] = NULL;
    dllpsirf_frame_hook_count = 0;
...
{
    if(dllpsirf_frame_hook_count < 2)
    {
        dllpsirf_frame_hook[dllpsirf_frame_hook_count] = frame_hook;
        dllpsirf_frame_hook_count++;
    }
}
//------------------------------------------------------------------------------------------------//
...
{
    return (BOOL)(dllpsirf_state != DLL_PSIRF_DISABLED);
}
//------------------------------------------------------------------------------------------------//
