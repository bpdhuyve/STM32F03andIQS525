...
static void AlP2P_CheckResponseCallback(COMMALP2P_REQUEST* request_ptr);
...
static void Command_CommAlP2PInfo(void)
{
    ...
    for(request_ptr = commalp2p_request; request_ptr < &commalp2p_request[commalp2p_request_count]; request_ptr++)
    {
        LOG_TRM("%3d | %3d | %6s | %6d | %6d", PU8(request_ptr->service),
                                                    ...
                                                    PCSTR(false_true[(request_ptr->ack_countdown > 0) || (request_ptr->response_countdown > 0)]),
...
static void AlP2P_CheckResponseCallback(COMMALP2P_REQUEST* request_ptr)
{
    ...
    {
        LOG_ERR("request_ptr->response_callback has become NULL");
    }
}
//------------------------------------------------------------------------------------------------//
static void AlP2P_OnFrameReceived(U8* data_ptr, U8 data_len)
           ...
           (request_ptr->request_id == request_id))
        {
            ...
            case P2P_MSG_FUNC_ACKNOWLEDGE:
                ...
                if(request_ptr->ack_countdown > 0)
                {
                    ...
                    {
                        AlP2P_CheckResponseCallback(request_ptr);
                        request_ptr->response_callback(RESPONSE_STATE_MESSAGE_SENT, NULL, 0);
                    }
                }
                break;
            ...
            case P2P_MSG_FUNC_RESPONSE:
                ...
                if(request_ptr->response_countdown > 0)
                {
                    ...
                    AlP2P_CheckResponseCallback(request_ptr);
                    request_ptr->response_callback(RESPONSE_STATE_RESPONSE_RECEIVED, &data_ptr[2], data_len - 2);
                }
                break;
            ...
            case P2P_MSG_FUNC_NOTAVAILABLE:
                ...
                if((request_ptr->ack_countdown > 0) || (request_ptr->response_countdown > 0))
                {
                    ...
                    request_ptr->response_countdown = 0;
                    AlP2P_CheckResponseCallback(request_ptr);
                    request_ptr->response_callback(RESPONSE_STATE_SERVICE_NA, NULL, 0);
                }
                break;
            }
        }
...
static void AlP2P_TimeoutTask(VPTR data_ptr)
{
    ...
    for(request_ptr = commalp2p_request; request_ptr < &commalp2p_request[commalp2p_request_count]; request_ptr++)
    {
        ...
        if(request_ptr->ack_countdown > 0)
        {
            if(request_ptr->ack_countdown == 1)
            {
                request_ptr->response_countdown = 0;
                AlP2P_CheckResponseCallback(request_ptr);
                request_ptr->response_callback(RESPONSE_STATE_ACKOWLEDGE_TO, NULL, 0);
            }
            request_ptr->ack_countdown--;
        }
        // else if waiting for response
        else if(request_ptr->response_countdown > 0)
        {
            if(request_ptr->response_countdown == 1)
            {
                AlP2P_CheckResponseCallback(request_ptr);
                request_ptr->response_callback(RESPONSE_STATE_RESPONSE_TO, NULL, 0);
            }
            request_ptr->response_countdown--;
        }
    }
}
//------------------------------------------------------------------------------------------------//
