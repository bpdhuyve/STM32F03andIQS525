...
static BOOL SendCommand(STRING command)
{
    ...
    LOG_DBG("Tx to bt chip:   \"%c\"", PDSTR(device_tx_string));
...
//------------------------------------------------------------------------------------------------//
BOOL CommBtUart_Send(STRING data)
{
...
//------------------------------------------------------------------------------------------------//
/// @brief  change the man in the midle protecton (mdesign app gebruikt dit, dan moet je een passkey ingeven, de demos gebruiken dit niet
/// @remark do this command first because it looks like it resets all the other stuff like the BT name ect
/// @param  "mitm_on" on or of, default without using this command is ON !!!
