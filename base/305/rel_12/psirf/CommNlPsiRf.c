...
typedef struct
{
    U8      master_rssi_node_to_hop;
    union
    {
        U8      master_consec_fail_counter;
        U8      slave_link_fail_value;
    }
    fail;
}
NL_PSIRF_NODE_INFO;
...
static void UpdateSlaveLinkValue(U8 node, U8 value);
...
static void NlPsiRfRegisterRouteInclusion(U8 new_node, U8 send_node, U8 rssi_send_to_master, U8 rssi_new_to_send);
static void NlPsiRfRegisterRoute(NL_PSIRF_MSG* msg_ptr, U8 rssi);
static U8 NlPsiRfLinkSuccessBasedOnRssi(U8 rssi);
...
static void CommNlPsiRf_Handler(void);
//================================================================================================//
...
static NLPSIRFHOOK_SEND_FRAME           nlpsirf_send_frame_hook;
...
static NL_PSIRF_NODE_INFO               nlpsirf_node_info[STDNLPSIRF_MAX_NODES];
...
static void UpdateSlaveLinkValue(U8 node, U8 value)
{
    U16 temp_u16 = (U16)nlpsirf_node_info[node].fail.slave_link_fail_value;
    temp_u16 *= 3;
    temp_u16 += value;
    temp_u16 >>= 2;
    nlpsirf_node_info[node].fail.slave_link_fail_value = (U8)temp_u16;
}
//------------------------------------------------------------------------------------------------//
static void NlPsiRfOnTimeOut(VPTR data_ptr)
{
    ...
    case NL_PSIRF_WAIT_FOR_EXPLORE_TIMEOUT:
        ...
        if(nlpsirf_network_hid_reveal > 0)
        {
            ...
            nlpsirf_outgoing_msg.tnid = NlPsiRfDetermineRouteTo(STDNLPSIRF_NETWORK_MASTER_ADDR);
            nlpsirf_outgoing_msg.rtnid = STDNLPSIRF_NETWORK_MASTER_ADDR;
...
//            nlpsirf_outgoing_msg.fid = NlPsiRfGetNextP2PFid();
        ...
        else
        {
            ...
            if(nlpsirf_data_ptr->network_address == STDNLPSIRF_NETWORK_MASTER_ADDR)
            {
                // indicate failed route
                if((++nlpsirf_node_info[nlpsirf_outgoing_msg.rtnid].fail.master_consec_fail_counter & 0x1F) == 0x10)
                {
                    LOG_TRM("Search %d (0x%02x)", PU8(nlpsirf_outgoing_msg.rtnid), PU8(nlpsirf_node_info[nlpsirf_outgoing_msg.rtnid].fail.master_consec_fail_counter));
                    CommNlPsiRf_SetLink(nlpsirf_outgoing_msg.rtnid);
                }
            }
            else
            {
                // update nlpsirf_node_info[tnid].fail.slave_link_fail_value
                UpdateSlaveLinkValue(nlpsirf_outgoing_msg.tnid, 100);
            }
            //enter state
...
static void NlPsiRfSendMsg(NL_PSIRF_MSG* msg_ptr)
{
    ...
    if(nlpsirf_send_frame_hook != NULL)
    {
        CommNlPsiRfUtil_DebugPrintMessage(MSG_DIR_SEND, msg_ptr, 0xFF);
        
        if(CommNlPsiRfUtil_ConvertMsgStructToArray(msg_ptr, data))
        {
            nlpsirf_send_frame_hook(data);
        }
    }
}
//------------------------------------------------------------------------------------------------//
static void NlPsiRfRemoveAddress(U8 address)
{
    ...
    if(address > STDNLPSIRF_NETWORK_MASTER_ADDR)
    {
        ...
        nlpsirf_data_ptr->route[address].rssi_to_node               = 0;
        nlpsirf_data_ptr->route[address].path.master_hop_via_node   = 0;
        nlpsirf_node_info[address].master_rssi_node_to_hop          = 0;
        nlpsirf_node_info[address].fail.master_consec_fail_counter  = 0;
        
        for(i = 2; i < STDNLPSIRF_MAX_NODES; i++)
        {
            if(nlpsirf_data_ptr->route[i].path.master_hop_via_node == address)
            {
                nlpsirf_data_ptr->route[i].path.master_hop_via_node = i;
                nlpsirf_node_info[i].master_rssi_node_to_hop        = 0xFF;
            }
        }
    }
}
...
static void NlPsiRfClearRoutingTable(void)
{
    MEMSET((VPTR)nlpsirf_data_ptr->route, 0, sizeof(nlpsirf_data_ptr->route));
}        
//------------------------------------------------------------------------------------------------//
static void NlPsiRfRegisterRouteInclusion(U8 new_node, U8 send_node, U8 rssi_send_to_master, U8 rssi_new_to_send)
{
    if(send_node == 0)  // direct sending = hopping via self
    {
        nlpsirf_data_ptr->route[new_node].rssi_to_node              = rssi_send_to_master;
        nlpsirf_data_ptr->route[new_node].path.master_hop_via_node  = new_node;
        nlpsirf_node_info[new_node].master_rssi_node_to_hop         = 0xFF;
        nlpsirf_node_info[new_node].fail.master_consec_fail_counter = 0;
    }
    else
    {
        nlpsirf_data_ptr->route[send_node].rssi_to_node             = rssi_send_to_master;
        nlpsirf_data_ptr->route[new_node].path.master_hop_via_node  = send_node;
        nlpsirf_node_info[new_node].master_rssi_node_to_hop         = rssi_new_to_send;
        nlpsirf_node_info[new_node].fail.master_consec_fail_counter = 0;
    }
    
    nlpsirf_wait_for_p2p_timeout_jitter ^= rssi_send_to_master;
}
//------------------------------------------------------------------------------------------------//
static void NlPsiRfRegisterRoute(NL_PSIRF_MSG* msg_ptr, U8 rssi)
{
    ...
    {
        // master
        if(msg_ptr->rsnid != nlpsirf_removed_nid)   // only register if real source node is not being removed
        {
            if(msg_ptr->tnid == STDNLPSIRF_NETWORK_MASTER_ADDR)
            {
                nlpsirf_data_ptr->route[msg_ptr->rsnid].path.master_hop_via_node    = msg_ptr->snid;
                nlpsirf_node_info[msg_ptr->rsnid].master_rssi_node_to_hop           = msg_ptr->rssi;
            }
        }
        if(msg_ptr->snid != nlpsirf_removed_nid)   // only register if source node is not being removed
        {
            if(nlpsirf_data_ptr->route[msg_ptr->snid].path.master_hop_via_node == 0)
            {
                nlpsirf_data_ptr->route[msg_ptr->snid].path.master_hop_via_node     = msg_ptr->snid;
                nlpsirf_node_info[msg_ptr->snid].master_rssi_node_to_hop            = 0xFF;
            }
            nlpsirf_data_ptr->route[msg_ptr->snid].rssi_to_node = rssi;
        }
    }
    else
    {
        // slave
        if(msg_ptr->rsnid == STDNLPSIRF_NETWORK_MASTER_ADDR)
        {
            nlpsirf_data_ptr->route[msg_ptr->snid].path.slave_rssi_master_to_hop = msg_ptr->rssi;
            if(nlpsirf_node_info[msg_ptr->snid].fail.slave_link_fail_value > 0)
            {
                nlpsirf_node_info[msg_ptr->snid].fail.slave_link_fail_value--;
            }
        }
        nlpsirf_data_ptr->route[msg_ptr->snid].rssi_to_node = rssi;
    }
...
{
    if(rssi == 0)   {return 0;}
    if(rssi == 255) {return 100;}
    if(rssi <= 112) {return 10;}
    if(rssi >= 152) {return 90;}
    
    return (10 + (rssi - 112)*2);
}
//------------------------------------------------------------------------------------------------//
static U8 NlPsiRfDetermineRouteTo(U8 rtnid)
{
    ...
    if((nlpsirf_data_ptr->network_address == STDNLPSIRF_NETWORK_MASTER_ADDR) && (rtnid > STDNLPSIRF_NETWORK_MASTER_ADDR))
    {
        if(nlpsirf_data_ptr->route[rtnid].path.master_hop_via_node > STDNLPSIRF_NETWORK_MASTER_ADDR)
        {
            return nlpsirf_data_ptr->route[rtnid].path.master_hop_via_node;
        }
    }
    else if((nlpsirf_data_ptr->network_address != STDNLPSIRF_NETWORK_MASTER_ADDR) && (rtnid == STDNLPSIRF_NETWORK_MASTER_ADDR))
    {
        // take rssi to master with PER taken into account
        rssi_reference = (U32)NlPsiRfLinkSuccessBasedOnRssi(nlpsirf_data_ptr->route[STDNLPSIRF_NETWORK_MASTER_ADDR].rssi_to_node);
        rssi_reference *= (U32)NlPsiRfLinkSuccessBasedOnRssi(nlpsirf_data_ptr->route[STDNLPSIRF_NETWORK_MASTER_ADDR].path.slave_rssi_master_to_hop);
        rssi_reference *= (U32)(100 - nlpsirf_node_info[STDNLPSIRF_NETWORK_MASTER_ADDR].fail.slave_link_fail_value);
        tnid = STDNLPSIRF_NETWORK_MASTER_ADDR;
        
        for(i = 2; i < STDNLPSIRF_MAX_NODES; i++)
        {
            rssi_via_hop = (U32)NlPsiRfLinkSuccessBasedOnRssi(nlpsirf_data_ptr->route[i].rssi_to_node);
            rssi_via_hop *= (U32)NlPsiRfLinkSuccessBasedOnRssi(nlpsirf_data_ptr->route[i].path.slave_rssi_master_to_hop);
            rssi_via_hop *= (U32)(100 - nlpsirf_node_info[i].fail.slave_link_fail_value);
            
            if(rssi_via_hop > rssi_reference)
            {
                rssi_reference = rssi_via_hop;
                tnid = i;
            }
        }
        
        if(nlpsirf_node_info[tnid].fail.slave_link_fail_value >= 95) // 97 is highest value reachable by filter
        {
            LOG_DBG("NL - Search Neighbours (%d)", PU32(rssi_reference));
            // reset all link fail values
            for(i = 0; i < STDNLPSIRF_MAX_NODES; i++)
            {
                nlpsirf_node_info[i].fail.slave_link_fail_value = 0;
            }
            // send explore
...
static NL_PSIRF_MSG_CATEGORY NlPsiRfCategorizeMsg(NL_PSIRF_MSG* msg_ptr)
{
    NL_PSIRF_MSG_CATEGORY_INFO  info;
    
    info.linking_active     = nlpsirf_in_linking;
    info.network_address    = nlpsirf_data_ptr->network_address;
    info.network_hid        = nlpsirf_data_ptr->network_hid;
    info.device_hid         = nlpsirf_data_ptr->device_hid;
    info.broadcast_msg_fid  = nlpsirf_broadcast_msg.fid;
    
    return CommNlPsiRfUtil_CategorizeMsg(msg_ptr, &info);
}
//------------------------------------------------------------------------------------------------//
static void NlPsiRfComposeAck(NL_PSIRF_MSG* msg_ptr)
{
...
static void NlPsiRfComposeSetNid(NL_PSIRF_MSG* msg_ptr)
{
    ...
    msg_ptr->snid = STDNLPSIRF_NETWORK_MASTER_ADDR;
    msg_ptr->rsnid = STDNLPSIRF_NETWORK_MASTER_ADDR;
...
static BOOL NlPsiRfAllowedToSendMsg(NL_PSIRF_MSG_SEND* outbox_msg_ptr)
{
    ...
    else if(nlpsirf_state == NL_PSIRF_IDLE)
    {
        ...
        case MSG_TYPE_V0_LINK:
            return (BOOL)((nlpsirf_data_ptr->network_address == STDNLPSIRF_NETWORK_MASTER_ADDR) || (outbox_msg_ptr->rtnid != 0));
        case MSG_TYPE_V0_REM_NID:
            return (BOOL)(nlpsirf_data_ptr->network_address > STDNLPSIRF_NETWORK_MASTER_ADDR);
...
static BOOL NlPsiRfHandleReceivedMsg(U8* frame_ptr, U8 rssi)
{
    ...
    if(CommNlPsiRfUtil_ConvertMsgArrayToStruct(frame_ptr, &incoming_msg))
    {
        CommNlPsiRfUtil_DebugPrintMessage(MSG_DIR_RECV, &incoming_msg, rssi);
...
#if STDNLPSIRF_ENABLE_REPEATER
            if(incoming_msg.snid == STDNLPSIRF_NETWORK_MASTER_ADDR)
            {
        ...
        case MSG_CATEGORY_P2P_SELF:
            ...
            case MSG_TYPE_V0_SET_NID:
                ...
                if(nlpsirf_data_ptr->network_address != STDNLPSIRF_NETWORK_MASTER_ADDR)
                {
                    // if remove from network message
                    if((incoming_msg.length <= STDNLPSIRF_HEADER_LENGTH_INCL) || (incoming_msg.data[STDNLPSIRF_HEADER_EXTRA_INCL] == 0))
                    {
                        // only accept when expecting to be removed (in wait for exclusion state)
                        // or when in linking and message does not resemble an expected ACK
                        if((nlpsirf_state == NL_PSIRF_WAIT_FOR_EXCLUSION) ||
                           ((nlpsirf_in_linking == TRUE) && ((incoming_msg.length > STDNLPSIRF_HEADER_LENGTH_INCL) ||
                                                             (nlpsirf_state != NL_PSIRF_WAIT_FOR_ACK) ||
                                                             (nlpsirf_outgoing_msg.rtnid != incoming_msg.rsnid) ||
                    ...
                    // otherwise if add to network message
                    else if(incoming_msg.data[STDNLPSIRF_HEADER_EXTRA_INCL] > STDNLPSIRF_NETWORK_MASTER_ADDR)
                    {
                        // if expected to be added
                        if(nlpsirf_state == NL_PSIRF_WAIT_FOR_INCLUSION)
                        {
                            //exit current state
                            NlPsiRfOnExit();
                        ...
                        // else if add message was re-send (e.g. because previous ACK was not received by master)
            ...
            case MSG_TYPE_V0_ACK:
                   ...
                   (nlpsirf_outgoing_msg.fid == incoming_msg.fid))
                {
                    ...
                    else
                    {
                        if(nlpsirf_data_ptr->network_address == STDNLPSIRF_NETWORK_MASTER_ADDR)
                        {
                            nlpsirf_node_info[nlpsirf_outgoing_msg.rtnid].fail.master_consec_fail_counter = 0;
                        }
                        else
                        {
                            // on slave: if successfull request to enter/leave linking mode: consider it done
                            if((nlpsirf_outgoing_msg.msg_type.v0 == MSG_TYPE_V0_LINK) && (nlpsirf_outgoing_msg.data[0] <= 1))
                            {
                                NlPsiRfSetLinking((BOOL)nlpsirf_outgoing_msg.data[0]);
                            }
                            // nlpsirf_node_info[tnid].fail.slave_link_fail_value
                            UpdateSlaveLinkValue(nlpsirf_outgoing_msg.tnid, 0);
                        }
                        //enter state
                        if(nlpsirf_fast_p2p == TRUE)
                        {
            ...
            case MSG_TYPE_V0_LINK:
                ...
                else if(nlpsirf_data_ptr->network_address == STDNLPSIRF_NETWORK_MASTER_ADDR)
                {
                    if(incoming_msg.data[0] <= 1)
                    {
                        ...
                        // always broadcast status again as response
                    ...
                    else if(nlpsirf_removed_nid == incoming_msg.rsnid)
                    {
                        ...
                        nlpsirf_inclusion_retry = INCLUSION_RETRY;
                        NlPsiRfOnEntry(NL_PSIRF_WAIT_FOR_ACK_SET_NID, 0);
                        // return, no ack needed, node will be removed
                        response_available = TRUE;
                        break;
                    }
                }
        ...
        case MSG_CATEGORY_NETW:
            NlPsiRfRegisterRoute(&incoming_msg, rssi);      // master & slave NETW msg
            break;
        ...
        case MSG_CATEGORY_EXPLORE:
            ...
            nlpsirf_reveal_msg.rsnid = STDNLPSIRF_NETWORK_MASTER_ADDR;
            nlpsirf_reveal_msg.rtnid = 0;
            nlpsirf_reveal_msg.fid = NlPsiRfGetNextBcFid();
            nlpsirf_reveal_msg.rssi = nlpsirf_data_ptr->route[STDNLPSIRF_NETWORK_MASTER_ADDR].rssi_to_node;
        ...
        {
            CommNlPsiRfUtil_DebugPrintMessage(MSG_DIR_REPLY, &incoming_msg, 0xFF);
            if(CommNlPsiRfUtil_ConvertMsgStructToArray(&incoming_msg, frame_ptr))
            {
                // add some wait time to avoid sending right away
                if(nlpsirf_state == NL_PSIRF_IDLE)
                {
                    NlPsiRfOnEntry(NL_PSIRF_WAIT_FOR_KEEP_SILENT_TIMEOUT, 20000); 
                }
                return TRUE;
            }
        }
    ...
    if((nlpsirf_state == NL_PSIRF_NO_ADDRESS) || (nlpsirf_state == NL_PSIRF_IDLE))
    {
        ...
        if((msg_to_handle == TRUE) || (CoreQ_Read(nlpsirf_q_hndl, (VPTR)&outbox_msg, 1)))
        {
            ...
            case MSG_TYPE_V0_EXPLORE:
                ...
                nlpsirf_outgoing_msg.rsnid = STDNLPSIRF_NETWORK_MASTER_ADDR;
                nlpsirf_outgoing_msg.fid = NlPsiRfGetNextBcFid();
                ...
                if(nlpsirf_state == NL_PSIRF_NO_ADDRESS)
                {
                    // explore trying to enter a network
                    nlpsirf_outgoing_msg.hid = nlpsirf_data_ptr->device_hid;
                    nlpsirf_outgoing_msg.snid = STDNLPSIRF_NETWORK_MASTER_ADDR;
                    NlPsiRfSendMsg(&nlpsirf_outgoing_msg);
                    //enter state
                ...
                {
                    // explore trying to find neighbours in network
                    nlpsirf_outgoing_msg.rssi = nlpsirf_data_ptr->route[STDNLPSIRF_NETWORK_MASTER_ADDR].rssi_to_node;
                    NlPsiRfSendMsg(&nlpsirf_outgoing_msg);
                    //enter state
                    NlPsiRfOnEntry(NL_PSIRF_WAIT_FOR_KEEP_SILENT_TIMEOUT, ((U32)STDNLPSIRF_BC_TIMEOUT_US));
                }
            ...
            case MSG_TYPE_V0_LINK:
                ...
                if(nlpsirf_outgoing_msg.rtnid == 0)
                {
                    ...
                    for(i=STDNLPSIRF_NETWORK_MASTER_ADDR+1;i<STDNLPSIRF_MAX_NODES;i++)
                    {
                        nlpsirf_data_ptr->route[i].rssi_to_node = 0;
                    }
                    //enter state
            ...
            case MSG_TYPE_V0_SET_NID:
                ...
                if(outbox_msg.rtnid == 0)
                {
                    ...
                    NlPsiRfSendMsg(&nlpsirf_outgoing_msg);
                    NlPsiRfClearRoutingTable();
                    nlpsirf_data_ptr->route[STDNLPSIRF_NETWORK_MASTER_ADDR].rssi_to_node                = 0xFF;
                    nlpsirf_data_ptr->route[STDNLPSIRF_NETWORK_MASTER_ADDR].path.master_hop_via_node    = 1;
                    //enter state
                    NlPsiRfOnEntry(NL_PSIRF_WAIT_FOR_KEEP_SILENT_TIMEOUT, ((U32)STDNLPSIRF_BC_TIMEOUT_US * 2));
                }
                else
...
void CommNlPsiRf_Init(NLPSIRF_DATA* data_ptr)
{
    ...
    else
    {
        ...
        for(i=0; i<STDNLPSIRF_MAX_NODES; i++)
        {
            nlpsirf_wait_for_p2p_timeout_jitter ^= nlpsirf_data_ptr->route[i].rssi_to_node;
        }
    }
    
    CommNlPsiRf_RegisterSendFrameHook(CommDllPsiRf_SendFrame);
    CommNlPsiRf_RegisterRecvFrameHookTo(CommDllPsiRf_RegisterFrameHook);
    ...
    MEMSET((VPTR)nlpsirf_node_info, 0, SIZEOF(nlpsirf_node_info));
    if(nlpsirf_data_ptr->network_address == STDNLPSIRF_NETWORK_MASTER_ADDR)
    {
        nlpsirf_node_info[STDNLPSIRF_NETWORK_MASTER_ADDR].master_rssi_node_to_hop = 0xFF;
    }
    nlpsirf_removed_nid = 0;
    nlpsirf_outbox_msg_unhandled = FALSE;
    nlpsirf_save_after_keep_silent = FALSE;
    ...
    Core_RegisterModuleHandler(CommNlPsiRf_Handler);
...
{
    data_ptr->device_hid = device_hid;
    MEMSET((VPTR)data_ptr->route, 0, SIZEOF(data_ptr->route));
    
    if(as_master == TRUE)
    {
        data_ptr->network_address                   = STDNLPSIRF_NETWORK_MASTER_ADDR;
        data_ptr->network_hid                       = device_hid;
        data_ptr->route[1].rssi_to_node             = 0xFF;
        data_ptr->route[1].path.master_hop_via_node = 1;
    }
    else
    {
        data_ptr->network_address                   = 0;
        data_ptr->network_hid                       = 0;
    }
}
//------------------------------------------------------------------------------------------------//
...
{
    // always copy first 12 bytes. They will remain unchanged.
    MEMCPY((VPTR)data_ptr, (VPTR)old_data_ptr, 12);
}
//------------------------------------------------------------------------------------------------//
void CommNlPsiRf_RegisterSendFrameHook(NLPSIRFHOOK_SEND_FRAME send_frame_hook)
{
    nlpsirf_send_frame_hook = send_frame_hook;
}
//------------------------------------------------------------------------------------------------//
void CommNlPsiRf_RegisterRecvFrameHookTo(NLPSIRFHOOK_REGISTER_RECV_FRAME registration_hook)
{
    if(registration_hook != NULL)
    {
        registration_hook(NlPsiRfHandleReceivedMsg);
    }
}
//------------------------------------------------------------------------------------------------//
...
U8 CommNlPsiRf_SetLink(U8 state)
{
    ...
    if((nlpsirf_data_ptr->network_address == STDNLPSIRF_NETWORK_MASTER_ADDR) && (state <= 1))
    {
    ...
    else
    {
        ...
        {
            msg_to_send.rtnid = STDNLPSIRF_NETWORK_MASTER_ADDR;
        }
...
U8 CommNlPsiRf_SendSetNidToZero(U8 nid)
{
    ...
    if((nlpsirf_data_ptr->network_address == STDNLPSIRF_NETWORK_MASTER_ADDR) && CoreQ_Write(nlpsirf_q_hndl, (VPTR)&msg_to_send, 1))
    {
        return msg_to_send.msg_id;
    }
    return 0;
...
{
    return nlpsirf_data_ptr->network_hid;
}
//------------------------------------------------------------------------------------------------//
...
void CommNlPsiRf_PrintStatus(void)
{
    ...
    else if(nlpsirf_data_ptr->network_address == 1)
    {
        // master
        LOG_TRM("NL Network     :  1 - 0xFF ( 1 - 0xFF)");
        for(i = 2; i < STDNLPSIRF_MAX_NODES; i++)
        {
            if((nlpsirf_data_ptr->route[i].rssi_to_node != 0) ||
               (nlpsirf_data_ptr->route[i].path.master_hop_via_node != 0) ||
               (nlpsirf_node_info[i].master_rssi_node_to_hop != 0))
            {
                LOG_TRM("                 %2d - 0x%02x (%2d - 0x%02x) F:0x%02x",
                        PU8(i),
                        PU8(nlpsirf_data_ptr->route[i].rssi_to_node),
                        PU8(nlpsirf_data_ptr->route[i].path.master_hop_via_node),
                        PU8(nlpsirf_node_info[i].master_rssi_node_to_hop),
                        PU8(nlpsirf_node_info[i].fail.master_consec_fail_counter));
                CoreLog_Flush();
            }
        }
    }
    ...
    {
        // slave
        hopnode = NlPsiRfDetermineRouteTo(STDNLPSIRF_NETWORK_MASTER_ADDR);
        
        lqi = (U32)NlPsiRfLinkSuccessBasedOnRssi(nlpsirf_data_ptr->route[1].rssi_to_node);
        lqi *= (U32)NlPsiRfLinkSuccessBasedOnRssi(nlpsirf_data_ptr->route[1].path.slave_rssi_master_to_hop);
        lqi *= (U32)(100 - nlpsirf_node_info[1].fail.slave_link_fail_value);
        
        LOG_TRM("NL Network     :  1 - 0x%02x (0x%02x) F:%2d LQI:%6d %s",
                PU8(nlpsirf_data_ptr->route[1].rssi_to_node),
                PU8(nlpsirf_data_ptr->route[1].path.slave_rssi_master_to_hop),
                PU8(nlpsirf_node_info[1].fail.slave_link_fail_value),
                PU32(lqi),
                PCSTR(star[(BOOL)(1 == hopnode)]));
        for(i = 2; i < STDNLPSIRF_MAX_NODES; i++)
        {
            if((nlpsirf_data_ptr->route[i].path.slave_rssi_master_to_hop != 0) || (nlpsirf_data_ptr->route[i].rssi_to_node != 0))
            {
                lqi = (U32)NlPsiRfLinkSuccessBasedOnRssi(nlpsirf_data_ptr->route[i].rssi_to_node);
                lqi *= (U32)NlPsiRfLinkSuccessBasedOnRssi(nlpsirf_data_ptr->route[i].path.slave_rssi_master_to_hop);
                lqi *= (U32)(100 - nlpsirf_node_info[i].fail.slave_link_fail_value);
                
                LOG_TRM("                 %2d - 0x%02x (0x%02x) F:%2d LQI:%6d %s",
                        PU8(i),
                        PU8(nlpsirf_data_ptr->route[i].rssi_to_node),
                        PU8(nlpsirf_data_ptr->route[i].path.slave_rssi_master_to_hop),
                        PU8(nlpsirf_node_info[i].fail.slave_link_fail_value),
                        PU32(lqi),
...
void CommNlPsiRf_GetRouteInfo(U8 tnid, U8* rssi_to_node_ptr, U8* hop_via_node_ptr, U8* rssi_node_to_hop_ptr)
{
    if(nlpsirf_data_ptr->network_address == 0)
    {
        *rssi_to_node_ptr       = 0;
        *hop_via_node_ptr       = 0;
        *rssi_node_to_hop_ptr   = 0;
    }
    else if(nlpsirf_data_ptr->network_address == 1)
    {
        *rssi_to_node_ptr       = nlpsirf_data_ptr->route[tnid].rssi_to_node;
        *hop_via_node_ptr       = nlpsirf_data_ptr->route[tnid].path.master_hop_via_node;
        *rssi_node_to_hop_ptr   = nlpsirf_node_info[tnid].master_rssi_node_to_hop;
    }
    else
    {
        *rssi_to_node_ptr       = nlpsirf_data_ptr->route[tnid].rssi_to_node;
        *hop_via_node_ptr       = NlPsiRfDetermineRouteTo(tnid);
        *rssi_node_to_hop_ptr   = nlpsirf_data_ptr->route[*hop_via_node_ptr].path.slave_rssi_master_to_hop;
    }
}
//================================================================================================//
