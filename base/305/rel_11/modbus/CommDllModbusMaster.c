...
//================================================================================================//
#define MODBUS__COMMDLLMODBUSMASTER_C
//================================================================================================//
...
#ifndef MODBUS__COMMDLLMODBUSMASTER_LOG_LEVEL
	#define CORELOG_LEVEL               LOG_LEVEL_DEFAULT
#else
	#define CORELOG_LEVEL               MODBUS__COMMDLLMODBUSMASTER_LOG_LEVEL
#endif
//------------------------------------------------------------------------------------------------//
// @brief  Defines the time in 탎, before this time there must be an answer to a request ohterwhise you get timeout
#ifndef MODBUSMASTER_T_TIMEOUT
    #error "MODBUSMASTER_T_TIMEOUT not defined in AppConfig.h"
#endif
//------------------------------------------------------------------------------------------------//
// @brief  Defines the max length of one Modbus ADU (Application Data Unit)
#ifndef MODBUSMASTER_STDDLL_FRAME_LENGTH
    #error "MODBUSMASTER_STDDLL_FRAME_LENGTH not defined in AppConfig.h"
#endif
//------------------------------------------------------------------------------------------------//
// @brief  Defines if the CRC is done by calculation (save memory) or by CRC table (save time)
#ifndef USE_CRC_TABLE_CALCULATION
    #define USE_CRC_TABLE_CALCULATION               0
#endif
#ifdef MODBUSMASTER_STDDLL_CRC_TABLE_CALC
    #warning "MODBUSMASTER_STDDLL_CRC_TABLE_CALC defined in AppConfig is depricated, use USE_CRC_TABLE_CALCULATION instead"
#endif
//================================================================================================//
...
#include "modbus\CommDllModbusMaster.h"
//================================================================================================//
...
//------------------------------------------------------------------------------------------------//
static void CommDllModbusMaster_Handler(void);
...
static DLLMODBUSMASTER_RECV_FRAME_HOOK          commdllmodbus_frame_hook            = NULL;
static EVENT_CALLBACK                           commdllmodbus_timeout_hook          = NULL;
static EVENT_CALLBACK				            commdllmodbus_txcomplete_hook       = NULL;
static DLLMODBUSMASTER_ERROR_HANDLING_HOOK	    commdllmodbus_error_handeling_hook  = NULL;

static U8                               commdllmodbus_length;
static U8                               commdllmodbus_frame_bytes[MODBUSMASTER_STDDLL_FRAME_LENGTH];
...
static DLLMODBUSMASTER_ERROR			        commdllmodbus_error;

#if (USE_CRC_TABLE_CALCULATION == 1)
...
static void CommDllModbusMaster_Handler(void)
{
    ...
    case DLL_MODBUS_STATE_RX:
        if(commdllmodbus_new_data_ready)
        {
            ...
            if(commdllmodbus_length < 4)
            {
                ...
                commdllmodbus_error = DLLMODBUSMASTER_ERR_FRAME_RX_TOO_SHORT;
                break;
            }
            //CRC check
            ...
            if(((U8)(crc_calculation & 0x00FF)) != commdllmodbus_frame_bytes[commdllmodbus_length - 2])
            {
                LOG_DBG("CRC NOK");
                commdllmodbus_new_data_ready = FALSE;
                commdllmodbus_error = DLLMODBUSMASTER_ERR_FRAME_RX_CRC_ERROR_1;
                break;
            }
            else if(((U8)(crc_calculation >> 8)) != commdllmodbus_frame_bytes[commdllmodbus_length - 1])
            {
                LOG_DBG("CRC NOK");
                commdllmodbus_new_data_ready = FALSE;
                commdllmodbus_error = DLLMODBUSMASTER_ERR_FRAME_RX_CRC_ERROR_2;
                break;
            }
            ...
            else
            {
                ...
                commdllmodbus_error = DLLMODBUSMASTER_ERR_NO_FRAME_HANDLER;
            }
            commdllmodbus_new_data_ready = FALSE;
        }
        break;
    ...
    default:
        ...
        commdllmodbus_error = DLLMODBUSMASTER_ERR_UNKNOWN_STATE;
        break;
    }
    StdDllCheckError();
}
...
static U8 CommDllModbus_TxGetNextByte(U8* byte_ptr, U8 length)
{
    ...
    if(commdllmodbus_state != DLL_MODBUS_STATE_TX)
    {
        ...
        commdllmodbus_error = DLLMODBUSMASTER_ERR_TX_WHILE_NOT_IN_TX_MODE;
        return 0;
    }
...
static void CommDllModbus_RxNewByte(U8* byte_ptr, U8 length)
{
    ...
    if(commdllmodbus_state != DLL_MODBUS_STATE_RX)
    {
        ...
        commdllmodbus_error = DLLMODBUSMASTER_ERR_RX_WHILE_NOT_IN_RX_MODE;
        return;
    }
    ...
    if(commdllmodbus_new_data_ready)
    {
        ...
        commdllmodbus_error = DLLMODBUSMASTER_ERR_FRAME_RX_LOST;
        return;
    }
    if(!commdllmodbus_sci_tx_pipe_active)
    {
        while(length)
        {
            ...
            case 1://data frame: receive data
                if(input_length <= MODBUSMASTER_STDDLL_FRAME_LENGTH)
                {
                ...
                else
                {
                    ...
                    commdllmodbus_dec_state = 0;
                    commdllmodbus_error = DLLMODBUSMASTER_ERR_FRAME_RX_TOO_LONG;
                }
                break;
            default:
                LOG_ERR("Unknown state");
    ...
    else
    {
        ...
        commdllmodbus_error = DLLMODBUSMASTER_ERR_RX_WHILE_TX_BUSY;
    }
    return;
}
//------------------------------------------------------------------------------------------------//
...
static void CommDllModbus_3_5Task(VPTR data_ptr)
{
    ...
    else
    {
        ...
        commdllmodbus_error = DLLMODBUSMASTER_ERR_INFRAME_SILENT_INTERVALS;
    }
}
//------------------------------------------------------------------------------------------------//
static U16 CRCGeneration(U8* value_ptr, U8 length)
...
static void StdDllCheckError(void)
{
    if((commdllmodbus_error != DLLMODBUSMASTER_ERR_NO_ERROR) &&  (commdllmodbus_error_handeling_hook != NULL))
    {
        commdllmodbus_error_handeling_hook(commdllmodbus_error);
    }
    commdllmodbus_error = DLLMODBUSMASTER_ERR_NO_ERROR;
}
//================================================================================================//
...
void CommDllModbusMaster_Init(SCI_CHANNEL_HNDL channel_hndl, SCI_SPEED sci_speed)
{
    ...
    commdllmodbus_1_5_task = CoreTask_RegisterTask(time_1_5, CommDllModbus_1_5Task, NULL, 255, "Modbus_1_5_MASTER"); //1.5 char time
    commdllmodbus_3_5_task = CoreTask_RegisterTask(time_3_5, CommDllModbus_3_5Task, NULL, 255, "Modbus_3_5_MASTER"); //3.5 char time
    commdllmodbus_timeout_task = CoreTask_RegisterTask(MODBUSMASTER_T_TIMEOUT, CommDllModbus_Timeout, NULL, 255, "ModbusTimeOut");
    
    Core_RegisterModuleHandler(CommDllModbusMaster_Handler);
...
//------------------------------------------------------------------------------------------------//
void CommDllModbusMaster_SendFrame(U8* data_ptr, U8 length)
{
    U16 crc_calculation;
    U8  crc_low;
    U8  crc_high;
    ...
    {
        if((length + 2) <= MODBUSMASTER_STDDLL_FRAME_LENGTH) // +2 for CRC
        {
			commdllmodbus_state = DLL_MODBUS_STATE_HOLD;
			...
			LOG_DBG("send Modbus message: %02h", PU8A(commdllmodbus_frame_bytes, length+2));
        }
        else
        {
            LOG_WRN("SendFrame failed: length!");
            commdllmodbus_error = DLLMODBUSMASTER_ERR_FRAME_TX_TOO_LONG;
        }
    }
    else
    {
        LOG_WRN("SendFrame failed: tx-mode is not queued!");
        commdllmodbus_error = DLLMODBUSMASTER_ERR_FRAME_TX_LOST;
		commdllmodbus_state = DLL_MODBUS_STATE_RX;
		commdllmodbus_tx_allowed = TRUE;
    }
    StdDllCheckError();
