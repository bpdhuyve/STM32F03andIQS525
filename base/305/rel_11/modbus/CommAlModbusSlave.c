...
#ifdef MODBUS_USE_MODBUS_TCPIP
	#include "modbus\CommNlTcpIpModbusSlave.h"
#else
...
//------------------------------------------------------------------------------------------------//
static void CommAlModbusSlave_InstallHook(U8 func_code, TEST_HOOK hook1, TEST_HOOK hook2, DUMMY_HOOK modbus_hook);
...
static BOOL CommAlModbusSlave_FindFunctionCode(U8 id, MODBUS_INTRFC* id_info_ptr);
static void CommAlModbusSlave_HandleData(U8* data_ptr, U8  length, U8* reply_data_ptr, U8* reply_length_ptr);
//================================================================================================//
...
static MODBUS_INTRFC                    al_modbus_intrfc[MODBUS_INTRFC_COUNT];
...
static void CommAlModbusSlave_InstallHook(U8 func_code, TEST_HOOK hook1, TEST_HOOK hook2, DUMMY_HOOK modbus_hook)
{
    ...
    while(temp_ptr < &al_modbus_intrfc[MODBUS_INTRFC_COUNT])
    {
        if(temp_ptr->id == 0)
        {
            temp_ptr->id          = func_code;
...
static BOOL CommAlModbusSlave_CheckSubReq20(U8* data_ptr)
{
    ...
    for(i = byte_count, j = 0, length_temp = 0; i > 0; i = i - 7, j++)
    {
        ...
        //file_nb = (data_ptr[1] << 8) + data_ptr[2];
...
static BOOL CommAlModbusSlave_CheckObjId(U8* data_ptr)
{
    ...
    LOG_WRN("Error: The object id is is not supported");
    return FALSE;
}
//------------------------------------------------------------------------------------------------//
static BOOL CommAlModbusSlave_CheckDeviceIdCode(U8* data_ptr)
...
static void CommAlModbusSlave_HandleData(U8* data_ptr, U8  length, U8* reply_data_ptr, U8* reply_length_ptr)
{
    ...
    if(!CommAlModbusSlave_FindFunctionCode(func_type, &id_info))
    {
        //Function code is not supported
        *reply_data_ptr = func_type | 0x80;
        reply_data_ptr++;
    ...
    {
        *reply_data_ptr = id_info.id | 0x80;
        reply_data_ptr++;
        if(func_type != 0x2B)
        {
            *reply_data_ptr = 0x03;
    ...
    else if((id_info.hook2 != NULL) && (!id_info.hook2(++data_ptr)))
    {
        *reply_data_ptr = id_info.id | 0x80;
        reply_data_ptr++;
        if(func_type != 0x2B)
...
void CommAlModbusSlave_Init(void)
{
	...
	#ifdef MODBUS_USE_MODBUS_TCPIP
		CommNlTcpIpModbusSlave_RegisterRequestHook(CommAlModbusSlave_HandleData);
	#else
		CommNlModbusSlave_RegisterRequestHook(CommAlModbusSlave_HandleData);
	#endif
...
{
    CommAlModbusSlave_InstallHook(0x01, CommAlModbusSlave_CheckQuantityId1_2, CommAlModbusSlave_CheckRangeId, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookDiscrInp(ALHOOK_READ_DISCR_INP hook)
{
    CommAlModbusSlave_InstallHook(0x02, CommAlModbusSlave_CheckQuantityId1_2, CommAlModbusSlave_CheckRangeId, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookReadHoldReg(ALHOOK_READ_HOLD_REG hook)
{
    CommAlModbusSlave_InstallHook(0x03, CommAlModbusSlave_CheckQuantityId3_4, CommAlModbusSlave_CheckRangeId, (DUMMY_HOOK)hook);
}
 /*-------------------------------------------------------------------------------------------------------------------*/
void CommAlModbusSlave_InstallHookReadInpReg(ALHOOK_READ_INP_REG hook)
{
    CommAlModbusSlave_InstallHook(0x04, CommAlModbusSlave_CheckQuantityId3_4, CommAlModbusSlave_CheckRangeId, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookWrSingleCoils(ALHOOK_WR_SINGLE_COIL hook)
{
    CommAlModbusSlave_InstallHook(0x05, CommAlModbusSlave_CheckOutputValue, NULL, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookWrSingleHoldReg(ALHOOK_WR_SINGLE_HOLD_REG hook)
{
    CommAlModbusSlave_InstallHook(0x06, NULL, NULL, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookReadExcStatus(ALHOOK_READ_EXC_STATUS hook)
{
    CommAlModbusSlave_InstallHook(0x07, NULL, NULL, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookWrMultCoils(ALHOOK_WR_MULT_COIL hook)
{
    CommAlModbusSlave_InstallHook(0x0F, CommAlModbusSlave_CheckQuantityId15, CommAlModbusSlave_CheckRangeId, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookWrMultHoldReg(ALHOOK_WR_MULT_HOLD_REG hook)
{
    CommAlModbusSlave_InstallHook(0x10, CommAlModbusSlave_CheckQuantityId16, CommAlModbusSlave_CheckRangeId, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookReadFileRec(ALHOOK_READ_FILE_REC hook)
{
    CommAlModbusSlave_InstallHook(0x14, CommAlModbusSlave_CheckByteCount, CommAlModbusSlave_CheckSubReq20, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookWrFileRec(ALHOOK_WR_FILE_REC hook)
{
    CommAlModbusSlave_InstallHook(0x15, CommAlModbusSlave_CheckByteCount, CommAlModbusSlave_CheckSubReq21, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_InstallHookDeviceId(ALHOOK_DEVICE_ID hook)
{
    CommAlModbusSlave_InstallHook(0x2B, CommAlModbusSlave_CheckObjId, CommAlModbusSlave_CheckDeviceIdCode, (DUMMY_HOOK)hook);
}
//------------------------------------------------------------------------------------------------//
void CommAlModbusSlave_RegisterFunctionCode(U8 func_code, ALHOOK_SPECIFIC hook)
{
    CommAlModbusSlave_InstallHook(func_code, NULL, NULL, (DUMMY_HOOK)hook);
}
//================================================================================================//
