...
//================================================================================================//
#define MODBUS__COMMDLLMODBUSSLAVE_C
//================================================================================================//
...
#ifndef MODBUSSLAVE_T_TIMEOUT
    #error "MODBUSSLAVE_T_TIMEOUT not defined in AppConfig.h"
#endif
//------------------------------------------------------------------------------------------------//
// @brief  Defines the max length of one Modbus ADU (Application Data Unit)
#ifndef MODBUSSLAVE_STDDLL_FRAME_LENGTH
    #error "MODBUSSLAVE_STDDLL_FRAME_LENGTH not defined in AppConfig.h"
#endif
//------------------------------------------------------------------------------------------------//
// @brief  Defines if the CRC is done by calculation (save memory) or by CRC table (save time)
#ifndef MODBUSSLAVE_STDDLL_CRC_TABLE_CALC
    #error "MODBUSSLAVE_STDDLL_CRC_TABLE_CALC not defined in AppConfig.h"
#endif
//------------------------------------------------------------------------------------------------//
// @brief  Defines if background handler is registered automatically or not. If not, the application must do the call
#ifndef AUTO_REGISTER_BACKGROUND_HANLDER
    #define AUTO_REGISTER_BACKGROUND_HANLDER                1
#endif
//================================================================================================//
...
static void TransmitCompleetHook(void);
//================================================================================================//
...
static DLLMODBUSSLAVE_RECV_FRAME_HOOK   	commdllmodbus_frame_hook         		= NULL;
static EVENT_CALLBACK				    	commdllmodbus_txcomplete_hook    		= NULL;
static DLLMODBUSSLAVE_ERROR_HANDLING_HOOK	commdllmodbus_error_handeling_hook      = NULL;

static U8                               commdllmodbus_length;
static U8                               commdllmodbus_frame_bytes[MODBUSSLAVE_STDDLL_FRAME_LENGTH];
...
static DLLMODBUSSLAVE_ERROR				commdllmodbus_error;

static U8								transmit_counter = 0;

#if (MODBUSSLAVE_STDDLL_CRC_TABLE_CALC == 1)
...
static U8 CommDllModbus_TxGetNextByte(U8* byte_ptr, U8 length)
{
    ...
    if(commdllmodbus_state != DLL_MODBUS_STATE_TX)
    {
        ...
        commdllmodbus_error = DLLMODBUSSLAVE_ERR_TX_WHILE_NOT_IN_TX_MODE;
...
static void CommDllModbus_RxNewByte(U8* byte_ptr, U8 length)
{
    ...
    if(commdllmodbus_state != DLL_MODBUS_STATE_RX)
    {
        ...
        commdllmodbus_error = DLLMODBUSSLAVE_ERR_RX_WHILE_NOT_IN_RX_MODE;
        return;
    }
    ...
    if(commdllmodbus_new_data_ready)
    {
        ...
        commdllmodbus_error = DLLMODBUSSLAVE_ERR_FRAME_RX_LOST;
        return;
    }
    if(!commdllmodbus_sci_tx_pipe_active)
    {
        while(length)
        {
            ...
            case 1://data frame: receive data
                ...
                {
                    LOG_WRN("commdllmodbus_frame_nok (silent interval): %d",PU8(commdllmodbus_frame_nok));
                    commdllmodbus_dec_state	= 0;
                }		
                if(input_length <= MODBUSSLAVE_STDDLL_FRAME_LENGTH)
                {
					input_length++;
					*input_data_ptr = *byte_ptr;
					input_data_ptr++;
                ...
                else
                {
                    ...
                    commdllmodbus_dec_state = 0;
                    commdllmodbus_error = DLLMODBUSSLAVE_ERR_FRAME_RX_TOO_LONG;
					return;
                }
                break;
            default:
    ...
    else
    {
        ...
        commdllmodbus_error = DLLMODBUSSLAVE_ERR_RX_WHILE_TX_BUSY;
    }
    return;
}
//------------------------------------------------------------------------------------------------//
...
static void CommDllModbus_Timeout(VPTR data_ptr)
{
    ...
    commdllmodbus_error = DLLMODBUSSLAVE_ERR_INTERFRAME_TIMEOUT_REPLY_NOT_SENT;
}
//------------------------------------------------------------------------------------------------//
static void CommDllModbus_1_5Task(VPTR data_ptr)
{
...
static void CommDllModbus_3_5Task(VPTR data_ptr)
{
    ...
    else
    {
        ...
        commdllmodbus_error = DLLMODBUSSLAVE_ERR_INFRAME_SILENT_INTERVALS;
    }
    commdllmodbus_dec_state = 0;
}
//------------------------------------------------------------------------------------------------//
...
static void StdDllCheckError(void)
{
    if((commdllmodbus_error != DLLMODBUSSLAVE_ERR_NO_ERROR) &&  (commdllmodbus_error_handeling_hook != NULL))
    {
        commdllmodbus_error_handeling_hook(commdllmodbus_error);
    }
    commdllmodbus_error = DLLMODBUSSLAVE_ERR_NO_ERROR;
}
//------------------------------------------------------------------------------------------------//
static void TransmitCompleetHook(void)
{
	...
	{
		LOG_TRM("transmit counter: %d", PU8(transmit_counter));
	}
	transmit_counter = 0;
}
//================================================================================================//
...
void CommDllModbusSlave_Init(SCI_CHANNEL_HNDL channel_hndl, SCI_SPEED sci_speed)
{
	...
	CommDllModbusSlave_RegisterTxCompleteHook(TransmitCompleetHook);
    ...
    commdllmodbus_timeout_task = CoreTask_RegisterTask(MODBUSSLAVE_T_TIMEOUT, CommDllModbus_Timeout, NULL, 255, "ModbusTimeOut");
    
    #if AUTO_REGISTER_BACKGROUND_HANLDER
    {
        Core_RegisterModuleHandler(CommDllModbusSlave_Handler);
    }
    #endif
...
void CommDllModbusSlave_Handler(void)
{
    ...
    case DLL_MODBUS_STATE_RX:
        if(commdllmodbus_new_data_ready)
        {
            ...
            if(commdllmodbus_length < 4)
            {
                ...
                commdllmodbus_error = DLLMODBUSSLAVE_ERR_FRAME_RX_TOO_SHORT;
                break;
            }
            //CRC check
            ...
            {
                LOG_DBG("CRC NOK");
                commdllmodbus_error = DLLMODBUSSLAVE_ERR_FRAME_RX_CRC_ERROR_1;
                break;
            }
            else if(((U8)(crc_calculation >> 8)) != commdllmodbus_frame_bytes[commdllmodbus_length - 1])
            {
                LOG_DBG("CRC NOK");
                commdllmodbus_error = DLLMODBUSSLAVE_ERR_FRAME_RX_CRC_ERROR_2;
                break;
            }
            ...
            else
            {
                ...
                commdllmodbus_error = DLLMODBUSSLAVE_ERR_NO_FRAME_HANDLER;
            }
    ...
    default:
        ...
        commdllmodbus_dec_state = 0;
        commdllmodbus_state = DLL_MODBUS_STATE_RX;
        commdllmodbus_error = DLLMODBUSSLAVE_ERR_UNKNOWN_STATE;
        break;
    }
    StdDllCheckError();
}
...
//------------------------------------------------------------------------------------------------//
void CommDllModbusSlave_SendFrame(U8* data_ptr, U8 length)
{
    U16 crc_calculation;
    U8  crc_low;
    U8  crc_high;
    ...
    {
        if((length + 2) <= MODBUSSLAVE_STDDLL_FRAME_LENGTH) // +2 for CRC
        {
        ...
        else
        {
            ...
            commdllmodbus_error = DLLMODBUSSLAVE_ERR_FRAME_TX_TOO_LONG;
        }
    }
    else
    {
        LOG_WRN("SendFrame failed: tx-mode is not queued!");
        commdllmodbus_error = DLLMODBUSSLAVE_ERR_FRAME_TX_LOST;
    }
    StdDllCheckError();
}
//================================================================================================//
