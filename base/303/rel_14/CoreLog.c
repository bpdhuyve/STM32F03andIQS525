...
//------------------------------------------------------------------------------------------------//
#define LOG_DATA_LOST_STRING                "LOG DATA LOST (%d)" NEWLINE PROMPT
#define POWER_UP_STRING                     NEWLINE NEWLINE "POWER UP" NEWLINE PROMPT
//================================================================================================//
...
static void CoreLog_FillOutput(void)
{
    ...
    while((*corelog_item.string != 0) || (corelog_item.decode_action != NO_DECODE_ACTION))
    {
...
static void CoreLog_PrintSub(STRING str, U8 len, va_list va_arg_in)
{
    ...
    va_copy(va_arg_count, va_arg_in);
    i = 0;
    ...
    while(i < len)
    {
        buffer[0] = va_arg(va_arg_count, U32);
        add = buffer[0] & LOG_SIZE_MASK;
        if(buffer[0] & LOG_BIT_ARRAY)
        {
            data_qsize += (va_arg(va_arg_count, U32) * add) + 2;    // 2 + n*m bytes: type + count (n) + n * m data
        ...
        va_arg(va_arg_count, U32);     // dummy readout U32 data
    }
    va_end(va_arg_count);
    ...
    if(CoreQ_WriteAlloc(corelog_q_hndl, data_qsize + hdr_qsize , &start_index))
    {
        ...
        while(i < len)
        {
            buffer[0] = (PTYPE)va_arg(va_arg_in, U32);
            add = buffer[0] & LOG_SIZE_MASK;
            if(buffer[0] & LOG_BIT_ARRAY)
            {
                buffer[1] = (U8)va_arg(va_arg_in, U32);
                data_ptr = (U32)va_arg(va_arg_in, U32);
            ...
            else
            {
                ...
                case LOG_SIZE_U8:
                    ...
                    break;
                case LOG_SIZE_U16:
                    CoreConvert_U16ToU8Array((U16)va_arg(va_arg_in, U32), &buffer[1]);
                    break;
                case LOG_SIZE_U32:
                    CoreConvert_U32ToU8Array((U32)va_arg(va_arg_in, U32), &buffer[1]);
                    break;
                }
...
void CoreLog_Handler(void)
{
    ...
    if((*corelog_item.string == 0) && (corelog_item.decode_action == NO_DECODE_ACTION) && (CoreQ_GetCount(corelog_q_hndl) >= 5))
    {
