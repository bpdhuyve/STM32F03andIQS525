...
{
    //implementation to make it work, can be faster

    LIST_INFO_STRUCT*   sourcelist_ptr = &list_info_struct[source_list];
    LIST_INFO_STRUCT*   destinationlist_ptr = &list_info_struct[destination_list];

    //lists must have the same item size
    if (sourcelist_ptr->item_size != destinationlist_ptr->item_size)
    {
        return FALSE;
    }

    CoreList_Clear(destination_list);

    static U8 tempdata[255]; //create tempdata for one item on the stack (item lenth is U8 so 255 max), if we would use the heap we could only allocate howmuch we need

    U8 items_in_source_list  = CoreList_GetCount(source_list);
    for(U8 i = 0; i<items_in_source_list;i++)
    {
        if (CoreList_Read(source_list, i, tempdata) == FALSE)
        {
            return FALSE;
        }
        if(CoreList_Add(destination_list, tempdata)== FALSE)
        {
            return FALSE;
        }
    }
