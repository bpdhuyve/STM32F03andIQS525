...
static void CoreLog_Command_CoreLogLevel(void);
#endif
#endif
//================================================================================================//
...
void CoreLog_Init(SCI_CHANNEL_HNDL channel_hndl)
{
    ...
    CoreTerm_RegisterCommand("CoreLogLevel", "CORE set log level", 1, CoreLog_Command_CoreLogLevel, FALSE);
#endif
...
void CoreLog_Print(STRING str, U8 level, U8 len, ...)
{
    ...
    if((corelog_loglevel & level) > 0)
    {
        ...
        while(i < len)
        {
            buffer[0] = va_arg(argp, U8);
            add = buffer[0] & LOG_SIZE_MASK;
            if(buffer[0] & LOG_BIT_ARRAY)
            {
                data_qsize += (va_arg(argp, U8) * add) + 2;     // 2 + n*m bytes: type + count (n) + n * m data
                i += 3;                                         // 3 input parameters
                (U32)va_arg(argp, U32);         // dummy readout data_ptr
            }
            else
            {
                ...
                {
                case LOG_SIZE_U8:
                    (U8)va_arg(argp, U8);       // dummy readout U8 data
                    break;
                case LOG_SIZE_U16:
                    (U16)va_arg(argp, U16);     // dummy readout U16 data
                    break;
                case LOG_SIZE_U32:
                    (U32)va_arg(argp, U32);     // dummy readout U32 data
                    break;
                }
            }
        }
        ...
        if(CoreQ_WriteAlloc(corelog_q_hndl, data_qsize + hdr_qsize , &start_index))
        {
            ...
            while(i < len)
            {
                buffer[0] = (PTYPE)va_arg(argp, U8);
                add = buffer[0] & LOG_SIZE_MASK;
                if(buffer[0] & LOG_BIT_ARRAY)
                {
                    buffer[1] = (U8)va_arg(argp, U8);
                ...
                else
                {
                    ...
                    case LOG_SIZE_U8:
                        ...
                        break;
                    case LOG_SIZE_U16:
                        CoreConvert_U16ToU8Array((U16)va_arg(argp, U16), &buffer[1]);
                        break;
                    case LOG_SIZE_U32:
