...
static void Command_HeapInfo(void)
{
    ...
    while(((U8*)item_ptr >= heap_buffer) && ((U8*)item_ptr < &heap_buffer[HEAP_SIZE]))
    {
        ...
        item_ptr = (HEAP_INFO*)((VPTR)item_ptr + item_ptr->item_size);
#if INCLUDE_TRACE != 0
        CoreLog_Flush();
#endif
    }
...
VPTR CoreHeap_Alloc(U32 request_size)
{
    ...
    while(Heap_CheckIfValid(item_ptr))
    {
        ...
        if(item_ptr->is_taken == 0)
        {
            ...
            if(item_ptr->item_size == request_size)
            {
                ...
                // return pointer
                return ((VPTR)item_ptr + SIZEOF(HEAP_INFO));
            }
    ...
    if(Heap_CheckIfValid(item_ptr))
    {
        ...
        {
            // create new
            new_item_ptr = (HEAP_INFO*)((VPTR)item_ptr + request_size);
            ...
            // update next
            next_item_ptr = (HEAP_INFO*)((VPTR)new_item_ptr + new_item_ptr->item_size);
...
void CoreHeap_Free(VPTR data_ptr)
{
    ...
    // take item
    item_ptr = (HEAP_INFO*)((VPTR)data_ptr - SIZEOF(HEAP_INFO));
    ...
    next_item_ptr = (HEAP_INFO*)((VPTR)item_ptr + item_ptr->item_size);
    ...
    if(Heap_CheckIfValid(prev_item_ptr) && (prev_item_ptr->is_taken == 0))
    {
        ...
        // update new next
        next_item_ptr = (HEAP_INFO*)((VPTR)prev_item_ptr + prev_item_ptr->item_size);
        if(Heap_CheckIfValid(next_item_ptr))
        {
