...
void CoreTerm_Init(SCI_CHANNEL_HNDL channel_hndl)
{
    ...
    MEMSET((VPTR)coreterm_rx_command, 0, SIZEOF(coreterm_rx_command));
    MEMSET((VPTR)coreterm_temp_string, 0, SIZEOF(coreterm_temp_string));
    MEMSET((VPTR)coreterm_commands, 0, SIZEOF(coreterm_commands));
    MEMSET((VPTR)coreterm_cmd_hooks, 0, SIZEOF(coreterm_cmd_hooks));
#endif

#if TERM_LEVEL > TERM_LEVEL_NO_HELP
    MEMSET((VPTR)coreterm_cmd_arg_count, 0, SIZEOF(coreterm_cmd_arg_count));
...
BOOL CoreTerm_VerifyArgumentCount(U8 expected_count)
{
    ...
    CoreString_RemoveExcessiveChar(coreterm_rx_read_ptr,' '); //remove exessive spaces in command
    real_count = CoreString_CountChar(coreterm_rx_read_ptr, ' ');
