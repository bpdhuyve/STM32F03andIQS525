...
static void Command_CoreTaskInfo(void)
{
    ...
    LOG_TRM("CoreTask");
    for(i = 0; i < TASK_COUNT; i++, task_obj_ptr++)
    {
        if(task_obj_ptr->callback_func != NULL)
        {
            #if (INCLUDE_INFO_STRING == 1)
            {
...
U32 CoreTask_GetPeriod(TASK_HNDL task)
{
    ...
    return task_ptr->us_per_task_period;
}
//------------------------------------------------------------------------------------------------//
