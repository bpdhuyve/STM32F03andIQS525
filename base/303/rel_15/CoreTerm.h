...
//------------------------------------------------------------------------------------------------//
#define CMD_LEVEL_PUBLIC_BIT                0x01
#define CMD_LEVEL_PROTECT_BIT               0x02
//================================================================================================//
...
// @remark  Enumeration of possible show levels
typedef enum
{
    CMD_HIDDEN                      = 0x00,                                             // Is always available and appears in "help /all" (= FALSE)
    CMD_PUBLIC                      = CMD_LEVEL_PUBLIC_BIT,                             // Is always available and appears in "help" (= TRUE)
    CMD_HIDDEN_PROTECTED            = CMD_LEVEL_PROTECT_BIT,                            // Is available and appears in "help /all" when protection is removed
    CMD_PUBLIC_PROTECTED            = CMD_LEVEL_PUBLIC_BIT | CMD_LEVEL_PROTECT_BIT,     // Is available and appears in "help" when protection is removed
}
CMD_LEVEL;
//================================================================================================//
...
//------------------------------------------------------------------------------------------------//
// @remark  function to enable/disable protection (default is protection ON)
...
// @remark  functions are prototyped depending on terminal level
    ...
    #define CoreTerm_GetArgumentAsU16(x1) 0
    #define CoreTerm_GetArgumentAsS16(x1) 0
    ...
    #define CoreTerm_GetByteStream(x1,x2,x3) NULL
        ...
        #define CoreTerm_RegisterCommand(x1,x2,x3,x4,x5)        CoreTerm_RegisterCommandWrap(x1,x2,x3,x4, (CMD_LEVEL)x5)
    #else
        #define CoreTerm_RegisterCommand(x1,x2,x3,x4,x5)        CoreTerm_RegisterCommandWrap(x1,"",x3,x4, (CMD_LEVEL)x5)
    #endif
    
    // @remark  registers a command to the terminal
    void CoreTerm_RegisterCommandWrap(STRING command, STRING helpText, U8 numberOfArguments, EVENT_CALLBACK commandoCallback, CMD_LEVEL cmd_level);
