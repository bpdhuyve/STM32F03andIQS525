...
//------------------------------------------------------------------------------------------------//
U16 CoreQ_ReadSmart(Q_HNDL hndl, VPTR data_ptr, U16 max_size)
{
    U16 bytes_to_get = MIN(CoreQ_GetCount(hndl), max_size);
    
    if(bytes_to_get > 0 && CoreQ_Read(hndl, data_ptr, bytes_to_get))
    {
        return bytes_to_get;
    }    
    return 0;
