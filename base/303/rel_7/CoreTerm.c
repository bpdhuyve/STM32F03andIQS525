...
static U8                              	coreterm_command_count = 0;
...
static void EventHandler_CommandReceived(CHAR* coreterm_command_ptr, U8 length)
{
            ...
            #if (TERM_LEVEL > TERM_LEVEL_NO_HELP)
            {
                if(coreterm_cmd_arg_count[i] != CMD_VAR_ARGUMENTS)  //skip this check if it is a var arguments cmd
                {
                    ...
                    if(coreterm_cmd_arg_count[i] != temp_argument_count_found)
                    {
                        ...
                        CoreLog_Print("Incorrect Cmd Arguments (found %d, expected %d)" NEWLINE PROMPT_STRING, LOG_LEVEL_TERM, 3, PU8(temp_argument_count_found), PU8(coreterm_cmd_arg_count[i]));
                        return;
                    }
                }
            }
    ...
    {
        CoreString_SubString(coreterm_commands[j], coreterm_temp_string, 0, i+1);
        CoreString_ToLowerCase(coreterm_temp_string);
...
static void CoreTerm_PrintHelp(void)
{
    ...
    for(i = 0; i < coreterm_command_count; i++)
    {
        if(coreterm_cmd_arg_count[i] == CMD_VAR_ARGUMENTS)
        {
            temp = CoreString_GetLength(coreterm_commands[i]) + 4;
        }
        else
        {
            temp = CoreString_GetLength(coreterm_commands[i]) + (coreterm_cmd_arg_count[i]*2);
        }
		if(temp > max_cmd_len)
		{
			max_cmd_len = temp;
    ...
    for(i = 0; i < coreterm_command_count; i++)
    {
        coreterm_command = coreterm_commands[i];

        CoreString_CopyString(coreterm_command,coreterm_temp_string);
        CoreString_ToLowerCase(coreterm_temp_string);

        if(((show_all) || (coreterm_cmd_hidden[i] == 0)) && (CoreString_StartsWith(coreterm_temp_string, rx_string)))
        {
            ...
            else
            {
                ...
                for(j = 0; j < coreterm_cmd_arg_count[i]; j++)
                {
        ...
        if(coreterm_command_count < MAX_NUMBER_OF_COMMANDS)
        {
            ...
            for(j = coreterm_command_count; j > i; j--)
            {
                ...
                #if (TERM_LEVEL > TERM_LEVEL_NO_HELP)
                {
                    ...
                    coreterm_cmd_hidden[j] = coreterm_cmd_hidden[j-1];
                }
                #endif
                #if (TERM_LEVEL > TERM_LEVEL_NO_HELP_TEXT)
                {
            ...
            #if (TERM_LEVEL > TERM_LEVEL_NO_HELP)
            {
                ...
                coreterm_cmd_hidden[i] = (U8)(show == FALSE);
            }
            #endif
            #if (TERM_LEVEL > TERM_LEVEL_NO_HELP_TEXT)
            {
