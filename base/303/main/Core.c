...
//------------------------------------------------------------------------------------------------//
    ...
    #endif
    #ifndef COREFREERTOS_TERM_TASK_PRIORITY
        #define COREFREERTOS_TERM_TASK_PRIORITY             3
    #endif
...
int main(void)
{
    ...
    #ifdef USE_FREERTOS
    {
        ...
        xTaskCreate(TermTask, (const portCHAR*)"TERM", COREFREERTOS_TERM_MINIMAL_STACK_SIZE, NULL, COREFREERTOS_TERM_TASK_PRIORITY, &task);
    ...
    do
    {
        if(*callback_ptr == module_handler)
        {
            *callback_ptr = NULL;
            ...
            // Fill whole in core_module_handlers array
            callback_ptr++;
            while((callback_ptr < &core_module_handlers[MODULE_HANDLER_COUNT]) &&
                  (*callback_ptr != NULL))
            {
                *(callback_ptr - 1) = *callback_ptr;
                *callback_ptr = NULL;
                callback_ptr++;
            }
            return;
...
void Core_OnErrorHandler(void)
{
    #ifdef USE_FREERTOS
    {
        ...
        {
             vTaskSuspendAll(); // suspends the scheduler to stay in Core_ErrorBackground()
        }
    }
    #endif
