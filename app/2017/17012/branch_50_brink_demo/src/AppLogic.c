...
static void DecodeTouches(U16 x, U16 y, BOOL first_touch);
...
//        LOG_TRM("%c%c;%c%c", PU8((x_cor>>8) & 0xFF),PU8(x_cor), PU8(y_cor>>8),PU8(y_cor));
       ...
       DecodeTouches(x_cor, y_cor, !previous_touched);
       inactive_counter = 0;
...
static void DecodeTouches(U16 x, U16 y, BOOL first_touch)
{
    ...
    static S32 first_y = 0;
    static U8 fanstand = 0;

    if((x < 250) && (y < 250)) //button _topleft
    {

        AppLeds_SetLed(LED_1,(COLOR_RGB){0,255,0});
        AppLeds_SetLed(LED_12,(COLOR_RGB){0,255,0});

        AppLeds_SetLed(LED_9,(COLOR_RGB){0,0,0});
        AppLeds_SetLed(LED_10,(COLOR_RGB){0,0,0});
    }
    else if((x < 250) && (y > 650)) //button _bottonleft
    {
        AppLeds_SetLed(LED_1,(COLOR_RGB){0,0,0});
        AppLeds_SetLed(LED_12,(COLOR_RGB){0,0,0});

        AppLeds_SetLed(LED_9,(COLOR_RGB){255,0,0});
        AppLeds_SetLed(LED_10,(COLOR_RGB){255,0,0});
    }

    else if(x > 650) //slider right
    {
        //zet led terug op manueel
        AppLeds_SetLed(LED_1,(COLOR_RGB){0,0,0});
        AppLeds_SetLed(LED_12,(COLOR_RGB){0,0,0});
        ...
        {
            S32 distance_finger_traveled;
            distance_finger_traveled = first_y - y;
            if (distance_finger_traveled > 250)
            {
                if (fanstand < 5)
                {
                    fanstand++;
            ...
            else if (distance_finger_traveled < (-250))
            {
                if (fanstand > 0)
                {
                    fanstand--;
...
static void SetLeds(U16 stand)
{
    ...
    for (U8 i = 6; i >= 2; i--)
    {
        if (stand > 0)
        {
            AppLeds_SetLed((LEDS)(1<<i),(COLOR_RGB){100,155,255}); //on
            stand--;
        }
        else
        {
            AppLeds_SetLed((LEDS)(1<<i),(COLOR_RGB){0,0,0}); //off
        }
    }
