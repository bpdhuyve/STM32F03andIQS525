...
static void Command_GpioInit(void);
static void Command_GpioSet(void);
static void Command_GpioGet(void);
#endif
static void Command_GpioInitMask(void);
static void Command_GpioSetMask(void);
static void Command_GpioGetMask(void);
static BOOL CommandAid_GetPort(U8 argumentNumber, GPIO_PORT* port_ptr);
#endif
//================================================================================================//
...
static void Command_GpioInitMask(void)
{
    ...
    {
        argnb++;
        LOG_TRM("PORT %c : 0x%04h", PCHAR('A' + gpio_port), PU32(CoreTerm_GetArgumentAsU32(argnb)));
...
static void Command_GpioGetMask(void)
{
    ...
    while(CommandAid_GetPort(argnb, &gpio_port))
    {
        LOG_TRM("PORT %c : 0x%04h", PCHAR('A' + gpio_port), PU32(SysGpio_GetPinMask(gpio_port, 0xFFFF)));
        argnb++;
    }
    CoreTerm_PrintAcknowledge();
}
...
static BOOL CommandAid_GetPort(U8 argumentNumber, GPIO_PORT* port_ptr)
{
    ...
    {
        *port_ptr = (GPIO_PORT)CoreTerm_GetArgumentAsU32(0);
    }
    return TRUE;
}
#endif
