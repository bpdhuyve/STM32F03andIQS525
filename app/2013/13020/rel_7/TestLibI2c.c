...
static BOOL TestI2c_InitDevice(U8 address, U32 speed, I2C_CHANNEL i2c_channel)
{
    return (BOOL)(DrvI2cMasterDevice_Register(DrvI2cMasterChannelSysInt_Register(i2c_channel), address, speed) != INVALID_I2C_DEVICE_ID);
}
//------------------------------------------------------------------------------------------------//
static void Command_I2cDeviceInit(void)
{
...
static void Command_I2cDeviceReadRegister(void)
{
    ...
    if(DrvI2cMasterDevice_WriteData(i2c_device_id, &register_address, 1, TRUE))
    {
        if(DrvI2cMasterDevice_ReadData(i2c_device_id, &data_buffer, 1, TRUE))
        {
