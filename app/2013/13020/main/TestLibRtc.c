...
static void Command_RtcSetTime(void)
{
    ...
    CoreTerm_PrintFeedback(DrvRtc_SetLocalTime(test_lib_rtc_hndl, &rtc_time_temp));
}
//------------------------------------------------------------------------------------------------//
static void Command_RtcGetTime(void)
{
    if(TestLibRtc_PrintCurrentTime() == TRUE)
    {
...
//------------------------------------------------------------------------------------------------//
BOOL TestLibRtc_PrintCurrentTime()
{
	RTC_TIME rtc_time_temp;
	if(DrvRtc_GetLocalTime(test_lib_rtc_hndl, &rtc_time_temp) == TRUE)
	{
		return TestLibRtc_PrintTime(rtc_time_temp);
	}
	return FALSE;
}
//------------------------------------------------------------------------------------------------//
...
{
	LOG_TRM("[RTC] %s %02d/%02d/%04d %02d:%02d:%02d", 
			PCSTR(day_of_week_string[rtc_time_temp.day_of_week]),
